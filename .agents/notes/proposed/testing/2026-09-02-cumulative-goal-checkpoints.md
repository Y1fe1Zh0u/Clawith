...
Drone and GitHub Actions invoke one tracked shell entry that carries G000 and G001 into G002 in manifest order, then adds the complete Backend test suite required by the current checkpoint. Both workflows fetch full history. The CI entry creates a temporary detached worktree at the fixed legacy commit, installs that checkout into its own virtual environment, supplies explicit distinct reference and target persistence values, and passes the temporary worktree and its Python executable to the immutable-reference validator. This validation override never binds, releases, or rewrites the canonical manifest, and cleanup removes the temporary worktree after both success and failure.
