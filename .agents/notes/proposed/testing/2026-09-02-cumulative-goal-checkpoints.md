...
G003 approves the `run` and `context` contracts before their G005 implementation. G004 approves the `session`, `a2a`, `group`, `trigger`, `heartbeat`, and `channel` contracts before their G006 implementation. These are contract-only prerequisites; they do not move schema, services, APIs, or tests into the earlier Goal.
...
- G003 and G004 contain the exact contract-only approval rosters without changing their implementation-owner rosters.
