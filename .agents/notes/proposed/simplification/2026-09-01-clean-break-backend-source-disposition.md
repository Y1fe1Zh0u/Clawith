...
The target application does not seed product data, create tables, repair schema, or materialize Workspace and Soul paths during setup or process startup. `setup.sh` now synchronizes `backend/.env` from the target template, prepares only the `clawith_target` database, and installs Backend dependencies without executing application bootstrap code. `restart.sh` launches only one `app.main:app` worker and verifies `/api/health`; its PID evidence includes process start identity and command identity, stale or mismatched evidence is never signaled, and armed EXIT/INT/TERM cleanup removes evidence only after the owned child terminates. It does not start the Frontend, Docker, product workers, connectors, migrations, or legacy Runtime roles. The canonical container entrypoint follows the same single-worker boundary. Structural guards inspect executable Shell and YAML positions with chained assignment expansion rather than treating inert prose as execution. The legacy Alembic revision chain remains unchanged until G008 replaces it with the approved target baseline.

Every retained Backend database value in root, CI, CD, deploy, and Helm configuration now resolves to `clawith_target`. All Compose services require the explicit `deferred-product` profile, and Helm defaults `g002Deferred` to true and renders no resources. These configurations are not a supported G002 product entry. The three CI scripts that ran legacy fresh-database migration, product deployment, and cross-version upgrade flows are removed; Drone and GitHub Actions run current Backend architecture, full test, collection, Ruff, and Pyright gates instead. No G002 CI path runs Alembic, builds release artifacts, or deploys product containers.
