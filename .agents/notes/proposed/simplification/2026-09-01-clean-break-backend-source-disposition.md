...
Status: proposed — the capability disposition is agreed; target application composition and database infrastructure are implemented, and the legacy Agent execution, old Context, structured Experience, old Model/LLM, Persistent Task, old Tool, old Skill, dedicated OpenClaw/Gateway, old Agent Credential, overloaded old Agent aggregate and its remaining dedicated schema/API tests, old Identity/Tenant aggregate, old Auth, old SSO and Security facade, overloaded old Organization/Relationship, legacy Invitation, Onboarding, Directory, Focus, Notification, Published Page, Plaza, legacy Agent Template, AgentBay, Tenant Knowledge publication, Group/Participant, Schedule, Trigger/Webhook, Heartbeat, OKR, legacy Channel, legacy Workspace, legacy A2A collaboration, residual advanced API, legacy Activity transport, legacy Messages transport, mixed Platform Administration and Enterprise transports, the monolithic shared schema module, orphan Activity/Audit loggers and persistence, legacy Run/Settings persistence facades, old Middleware/Permission/Error compatibility and BaseDAO contract test, orphan global DAO substrate, orphan global logging configuration, legacy Platform URL service, legacy quota guard, legacy Realtime services, monolithic resource discovery, legacy System Email service, vision injection and orphan maintenance scripts, legacy Email attachment/storage coupling, legacy seed/bootstrap authority, legacy storage facade and fallback authority, and old Autonomy/Approval authorities are removed, while reviewed object-storage mechanics are isolated in infrastructure and owner rewrites and the remaining category deletions remain incomplete
