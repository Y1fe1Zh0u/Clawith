...
test("answer stream starts and resets provisional content by attempt", () => {
  ...
  assert.deepEqual(
    reduceSessionStreamChunk(firstAttempt, {
      run_id: "run-1",
...
test("answer stream appends only contiguous packets from the active attempt", () => {
  ...
  assert.equal(
    reduceSessionStreamChunk(replay, {
      run_id: "run-1",
      attempt_id: "attempt-1",
      sequence: 2,
      content: " duplicate",
...
test("failed cancelled and delivery-failed terminals preserve provisional output", () => {
  ...
  assert.equal(
    shouldPreserveInterruptedStream("completed", "delivery_failed"),
    true,
  );
...
test("session Tool cache restores a running card after switching back", () => {
  ...
  assert.deepEqual(
    ...
    [{ id: "user-1", role: "user", content: "move it" }, running],
  );
});
...
test("settled lane transition refreshes canonical messages after websocket loss", () => {
  ...
  assert.equal(
    runtimeCompletionNeedsMessageRefresh(waitingRun, waitingRun),
    false,
  );
});
...
test("poll-before-websocket ordering merges runtime diagnostics into the canonical message", () => {
  ...
  assert.deepEqual(
    mergeTerminalAssistantMessage([canonical], { ...canonical, runtimeError }),
...
test("workspace reconciliation fields preserve status and file counts", () => {
  ...
  assert.equal(
    toolReconciliationNeedsUserAction(parsed.pendingToolReconciliations[0]),
    true,
  );
  assert.equal(
    toolReconciliationsByCallId(parsed.pendingToolReconciliations).get("call-1")
      ?.executionId,
    "execution-1",
  );
});
...
test("workspace reconciliation keeps decisions in the composer and passive status on the tool row", () => {
  ...
  assert.doesNotMatch(
    agentDetailSource,
    /analysis-tool-reconciliation__actions/,
  );
});
...
test("onboarding only treats an authoritative runtime-state payload as loaded", () => {
  assert.equal(
    sessionRuntimeStateResponseIsValid({ active_run: null }, null),
    true,
  );
