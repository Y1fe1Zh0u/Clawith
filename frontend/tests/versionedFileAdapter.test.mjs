...
test("version snapshots distinguish a listed token from a path absent in the loaded directory", () => {
  const adapter = createVersionedFileAdapter({
    async read() {
      return { content: "", version_token: null };
    },
    async write(_path, content) {
      return { content, version_token: "v1" };
    },
    async delete() {},
  });
  ...
  assert.deepEqual(adapter.snapshot("existing.md"), {
    known: true,
    versionToken: "version-1",
  });
  assert.deepEqual(adapter.snapshot("new.md"), {
    known: false,
    versionToken: null,
  });
});
...
test("group workspace uploads stay in the current directory and decode exact UTF-8 text", async () => {
  assert.equal(
    groupWorkspaceUploadPath("reports/weekly", "summary.md"),
...
test("group workspace accepts the single-agent formats and classifies binary files", async () => {
  for (const extension of [
    ...
    assert.equal(
      groupWorkspaceUploadPath("shared", `sample${extension}`),
      `shared/sample${extension}`,
    );
...
test("group workspace upload rejects traversal, unsupported formats, and invalid UTF-8", async () => {
  assert.throws(
    ...
    (error) =>
      error instanceof GroupWorkspaceUploadError &&
