...
test("onboarding kickoff waits for empty history and authoritative idle runtime state", () => {
  ...
  assert.equal(
    shouldKickoffOnboarding({ ...ready, messagesLoaded: false }),
    false,
  );
  assert.equal(
    shouldKickoffOnboarding({ ...ready, runtimeStateLoaded: false }),
    false,
  );
  assert.equal(shouldKickoffOnboarding({ ...ready, messageCount: 1 }), false);
  assert.equal(
    shouldKickoffOnboarding({ ...ready, hasActiveRun: true }),
    false,
  );
});
