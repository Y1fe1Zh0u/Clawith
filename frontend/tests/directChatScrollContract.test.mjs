...
test("direct chat keeps document scrolling separate from history pagination", () => {
  ...
  assert.equal(
    (agentDetail.match(/useOlderHistoryGesture\(/g) ?? []).length,
    2,
  );
  assert.equal(
    (agentDetail.match(/usePrependScrollAnchor\(/g) ?? []).length,
    2,
  );
  assert.equal(
    (agentDetail.match(/loadDirectHistoryTurn(?:<any>)?\(/g) ?? []).length,
    2,
  );
  assert.equal(
    (
  ...
  assert.match(
    styles,
    /\.agent-chat-message-scroll\s*\{[^}]*overflow-anchor:\s*none;/s,
  );
});
