...
test("legacy detail supports strings, structured objects, and validation lists", () => {
  const stringError = parseHttpError({
    status: 400,
    bodyText: JSON.stringify({ detail: "Bad input" }),
  });
  ...
  const objectError = parseHttpError({
    status: 403,
    bodyText: JSON.stringify({ detail: objectDetail }),
  });
...
test("object details never degrade to object stringification when a message exists", () => {
  const error = parseHttpError({
    status: 400,
    bodyText: JSON.stringify({
      detail: { error: { message: "Nested backend message" } },
    }),
  });
...
test("plain text and empty responses receive useful messages", () => {
  assert.equal(
    parseHttpError({ status: 502, bodyText: "Upstream unavailable" }).message,
    "Upstream unavailable",
  );
  assert.equal(
    parseHttpError({ status: 404, statusText: "Not Found", bodyText: "" })
      .message,
    "HTTP 404 Not Found",
  );
});
...
test("unknown thrown values normalize to typed AppError instances", () => {
  ...
  const object = normalizeUnknownError(
...
test("normal requests and both upload transports use the shared parser", () => {
  ...
  const enterpriseFetcher = readFileSync(
    new URL(
      "../src/pages/enterprise-settings/utils/fetchJson.ts",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  const userManagement = readFileSync(
    new URL("../src/pages/UserManagement.tsx", import.meta.url),
    "utf8",
  );
