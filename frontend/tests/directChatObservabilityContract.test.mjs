...
test("direct chat runtime controls and delayed sends stay scoped to the selected session", () => {
  ...
  assert.match(
    source,
    /if \(isActiveRuntime\) \{[\s\S]*setIsWaiting\(true\);[\s\S]*setChatMessages/,
  );
});
...
test("background Tool packets are cached per session and restored only when selected", () => {
  ...
  assert.ok(
    backgroundReturn > cacheWrite,
    "Tool packets must be cached before background UI events return",
  );
  assert.match(
    source,
...
test("direct chat renders canonical runtime diagnostics and keeps legacy fallbacks", () => {
  ...
  assert.match(
    source,
    /runtimeError: normalizeRuntimeError\(\{ error: (?:message|m)\.runtime_error \}\)/,
  );
});
...
test("runtime packet normalization prefers canonical context and supports legacy delivery errors", () => {
  ...
  assert.equal(
    runtimeErrorDisablesReconnect({ message: "No model", code: "other" }),
    false,
  );
  assert.equal(
    runtimeErrorDisablesReconnect({
