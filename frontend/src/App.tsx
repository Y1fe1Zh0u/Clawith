...
const notificationBarRevisionKey = (
  config: Pick<NotificationBarConfig, "text" | "updated_at">,
) => btoa(encodeURIComponent(`${config.text}::${config.updated_at || ""}`));
const notificationBarSessionDismissKey = (
  config: Pick<NotificationBarConfig, "text" | "updated_at">,
) => `notification_bar_dismissed_session_${notificationBarRevisionKey(config)}`;
const notificationBarPersistentDismissKey = (
  config: Pick<NotificationBarConfig, "text" | "updated_at">,
) =>
