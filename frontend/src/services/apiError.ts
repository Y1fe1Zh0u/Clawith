...
function messageFromValue(value: unknown): string | undefined {
  ...
  if (isRecord(value.error)) {
    const nested =
      optionalString(value.error.message) ?? optionalString(value.error.detail);
...
export function parseHttpError(input: HttpErrorInput): ApiError {
  ...
  const canonical =
    envelope && isRecord(envelope.error) ? envelope.error : undefined;
  ...
  const message =
    ...
    fallback;
  const details =
    canonical && "details" in canonical ? canonical.details : legacyDetail;
  const retryable = firstField(contextRecords, "retryable");

  return new ApiError({
    message,
    code:
      optionalString(firstField(contextRecords, "code")) ??
      (input.status ? `http_${input.status}` : "http_error"),
    status: input.status,
    traceId:
      optionalString(firstField(contextRecords, "trace_id", "traceId")) ??
      optionalString(input.traceId),
