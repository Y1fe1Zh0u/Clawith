...
export function normalizeRuntimeError(
  ...
  const canonical =
    packet.error && typeof packet.error === "object"
  ...
  return {
    message:
      ...
      "Request denied",
    code:
      text(canonical.code) || text(packet.code) || text(packet.delivery_error),
...
export function formatRuntimeErrorDiagnostics(
  error: RuntimeErrorContext,
): string {
  return [
...
export const runtimeErrorDisablesReconnect = (
  error: RuntimeErrorContext,
): boolean =>
...
export const runtimeErrorMarksAgentExpired = (
  error: RuntimeErrorContext,
): boolean => error.code === "agent_expired";
