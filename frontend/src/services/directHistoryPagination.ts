...
export async function loadDirectHistoryTurn<T extends DirectHistoryRow>({
  ...
  for (
    let requestCount = 1;
    requestCount <= maxContinuationPages;
    requestCount += 1
  ) {
    ...
    if (completeToolTurn) {
      ...
      if (userBoundaryIndex >= 0) {
        ...
        if (!boundaryCursor)
          throw new Error("Direct history boundary is missing its cursor");
        return {
    ...
    if (!nextCursor)
      throw new Error("Direct history page is missing its cursor");
    hasMore = batch.length >= pageSize;
    if (!completeToolTurn || !hasMore) {
      return { rows, oldestCursor: nextCursor, hasMore, requestCount };
    }
    if (seenCursors.has(nextCursor))
      throw new Error("Direct history cursor did not advance");
    seenCursors.add(nextCursor);
    cursor = nextCursor;
  }

  throw new Error(
    `Direct history Tool turn exceeded ${maxContinuationPages} continuation pages`,
  );
}
