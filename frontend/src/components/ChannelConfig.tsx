...
const CHANNEL_REGISTRY: ChannelDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "tenant_id",
        label: "channelGuide.teams.tenantId",
        placeholder: "channelGuide.teams.tenantIdPlaceholder",
      },
    ],
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        label: "AgentId",
        type: "text",
        placeholder: "DingTalk应用AgentId(可选)",
        required: false,
      },
    ],
...
export default function ChannelConfig({
  ...
  const [feishuPermMode, setFeishuPermMode] = useState<"basic" | "full">(
    "basic",
  );
  ...
  const setEditing = (id: string, val: boolean) =>
    setEditingChannels((prev) => ({ ...prev, [id]: val }));
  ...
  const setFormField = (channelId: string, key: string, val: string) =>
    setForms((prev) => ({
      ...prev,
      [channelId]: { ...prev[channelId], [key]: val },
    }));
  ...
  const togglePwd = (fieldId: string) =>
    setShowPwds((p) => ({ ...p, [fieldId]: !p[fieldId] }));
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (_d, { ch }) => {
      ...
      setActionFeedback({
        type: "success",
        text: t(
          "agent.settings.channel.saveSuccess",
    ...
    onError: (error: Error) => {
      setActionFeedback({
        ...
        text:
          error.message ||
          t(
            "agent.settings.channel.saveFailed",
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_d, { ch }) => {
      ...
      setActionFeedback({
        type: "success",
        text: t(
          "agent.settings.channel.disconnectSuccess",
    ...
    onError: (error: Error) => {
      setActionFeedback({
        ...
        text:
          error.message ||
          t(
            "agent.settings.channel.disconnectFailed",
  ...
  useEffect(() => {
    ...
    if (raw.startsWith("http://") || raw.startsWith("https://")) {
      const token = localStorage.getItem("token");
      fetch(
        `/api/agents/${agentId}/wechat-channel/qrcode-image?url=${encodeURIComponent(raw)}`,
        {
  ...
  const renderGuide = (guide: GuideConfig, isWs: boolean, ch: ChannelDef) => {
    const prefix =
      isWs && ch.wsGuide
        ? `${ch.wsGuide.prefix}.ws_step`
        : `${guide.prefix}.step`;
    const stepCount = isWs && ch.wsGuide ? ch.wsGuide.steps : guide.steps;
    const noteKey =
      isWs && ch.wsGuide
        ? `${ch.wsGuide.prefix}.ws_note`
        : guide.noteKey || `${guide.prefix}.note`;
      ...
      >
        ...
        {ch.showPermJson && (
          ...
          >
            ...
            >
              ...
              >
                {(["basic", "full"] as const).map((mode) => (
                  ...
                  >
                    {t(
                      `channelGuide.feishuPerm${mode === "basic" ? "Basic" : "Full"}`,
                    )}
                  </button>
                ))}
              </div>
            ...
            >
              {feishuPermMode === "basic"
                ? t("channelGuide.feishuPermBasicDesc")
                : t("channelGuide.feishuPermFullDesc")}
            </div>
            {/* JSON code block */}
            <pre
              style={{
  ...
  const renderField = (
    ...
    const labelText = field.label.startsWith("channelGuide.")
      ? t(field.label)
      : field.label;
    const placeholderText = field.placeholder?.startsWith("channelGuide.")
      ? t(field.placeholder)
      : field.placeholder;
          ...
          <input
            ...
            style={
              mode === "edit"
                ? {
                    fontSize: "12px",
                    paddingRight: isSecret ? "36px" : undefined,
                    width: "100%",
                  }
                : undefined
            }
  ...
  const renderCreateChannel = (ch: ChannelDef) => {
    ...
    const connMode = ch.connectionMode
      ? connectionModes[ch.id] || "websocket"
      : null;
    const isWs = connMode === "websocket";

    // Active fields for current mode
    const activeFields =
      ch.connectionMode && isWs && ch.wsFields ? ch.wsFields : ch.fields;
    ...
    const hasValues = formFields.some(
      (f) => f.required && values?.[`${ch.id}_${f.key}`],
    );
      ...
      >
        ...
        >
          ...
          {hasValues && (
            ...
            >
              {t("agent.settings.channel.configured", "Configured")}
            </span>
          )}
          <span
            style={{
        ...
        {isOpen && (
          <div style={{ padding: "16px" }}>
            ...
            {ch.connectionMode && (
              ...
              >
                <label
                  style={{ fontSize: "12px", fontWeight: 500, width: "120px" }}
                >
                  {t("agent.settings.channel.mode", "Connection Mode")}
                </label>
                <label
                  style={{
                    display: "flex",
  ...
  const renderEditChannel = (ch: ChannelDef) => {
    ...
    const isConfigured =
      ch.id === "feishu" ? config?.is_configured : config?.is_configured;
          ...
          >
            {!canManage ? (
              ...
              <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ color: "var(--text-secondary)" }}>
                        Connected via WebSocket (No callback URL needed)
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      App ID: <code>{config.app_id}</code>
                    </div>
                  </div>
                )}
                {ch.id === "feishu" && configConnMode !== "websocket" && (
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ color: "var(--text-secondary)" }}>
                        {config.is_connected
                          ? t(
                              "agent.settings.channel.websocketConnected",
                              "Connected via WebSocket (No callback URL needed)",
                            )
                          : t(
                              "agent.settings.channel.websocketDisconnected",
                              "Configured for WebSocket, but currently disconnected",
                            )}
                      </span>
                    </div>
                    {!config.is_connected && (
                      ...
                      >
                        {t(
                          "agent.settings.channel.websocketDisconnectedHint",
                          "Reconnect by saving the WeCom WebSocket configuration again.",
                        )}
                      </div>
                    )}
                  </div>
                ...
                {ch.webhookLabel &&
                  !(ch.connectionMode && configConnMode === "websocket") &&
                  ch.id !== "dingtalk" &&
                  ch.id !== "atlassian" && (
                    <div
                      style={{
                ...
                {ch.id === "discord" && configConnMode !== "gateway" && (
                  ...
                  >
                    Use <code>/ask message:&lt;your question&gt;</code> to talk
                    to this agent
                  </div>
                )}
                {ch.id === "discord" && configConnMode === "gateway" && (
                  ...
                  >
                    ...
                    >
                      ...
                      ></span>
                      <span style={{ color: "var(--text-secondary)" }}>
                        Connected via Gateway (No public URL needed)
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      @mention the bot or send a DM to interact
                    </div>
                  </div>
                )}
                ...
                {ch.id === "dingtalk" && configConnMode === "websocket" && (
                  ...
                  >
                    ...
                    >
                      ...
                      ></span>
                      <span style={{ color: "var(--text-secondary)" }}>
                        Connected via Stream (No callback URL needed)
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      App Key: <code>{config.app_id}</code>
                    </div>
                  </div>
                )}
                {ch.id === "dingtalk" && configConnMode !== "websocket" && (
                ...
                {ch.id === "atlassian" && (
                  ...
                  >
                    ...
                    >
                      API Key configured — Jira / Confluence / Compass tools
                      available
                    </div>
                    {config.cloud_id && (
                      <div
                      ...
                      >
                        Cloud ID: <code>{config.cloud_id}</code>
                      </div>
                    )}
                  </div>
                )}
                {ch.id === "wechat" && (
                  ...
                  >
                    ...
                    {config.app_id && (
                      ...
                      >
                        Bot ID: <code>{config.app_id}</code>
                      </div>
                    )}
                    {config.extra_config?.ilink_user_id && (
                      <div
                        style={{
                          color: "var(--text-tertiary)",
                          marginTop: "4px",
                      ...
                      >
                        Linked User:{" "}
                        <code>{config.extra_config.ilink_user_id}</code>
                      </div>
                    )}
                  </div>
                )}
                {ch.id === "atlassian" && atlassianTestResult && (
                  <div
                    style={{
                      ...
                      fontSize: "12px",
                      marginBottom: "10px",
                      background: atlassianTestResult.ok
                        ? "rgba(16,185,129,0.08)"
                        : "rgba(239,68,68,0.08)",
                      border: `1px solid ${atlassianTestResult.ok ? "rgba(16,185,129,0.25)" : "rgba(239,68,68,0.25)"}`,
                      color: atlassianTestResult.ok
                        ? "rgb(5,150,105)"
                        : "rgb(220,38,38)",
                    }}
                  >
                      ...
                      onClick={() => {
                        ...
                        if (ch.id === "feishu") {
                          ...
                          setConnectionModes((prev) => ({
                            ...prev,
                            feishu:
                              config.extra_config?.connection_mode ||
                              "websocket",
                          }));
                        } else if (ch.id === "wecom") {
                          const cm =
                            config.extra_config?.connection_mode === "websocket"
                              ? "websocket"
                              : "webhook";
                          setConnectionModes((prev) => ({
                            ...prev,
                          ...
                          if (cm === "websocket") {
                            ...
                            prefill.bot_secret =
                              config.extra_config?.bot_secret || "";
                          } else {
              ...
              >
                {ch.id === "wechat" ? (
                  <>
                    ...
                    >
                      This channel uses QR login and long polling. After scan
                      confirmation, the bot will start polling WeChat iLink
                      automatically.
                    </div>
                    {wechatQr?.qrcode_img_content ? (
                      <div
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                          <button
                            className="btn btn-secondary"
                            style={{ fontSize: "12px" }}
                            onClick={createWechatQr}
                            disabled={wechatLoadingQr}
                          >
                          ...
                          {isEditing && (
                            <button
                              className="btn btn-secondary"
                              style={{ fontSize: "12px" }}
                              onClick={() => setEditing(ch.id, false)}
                            >
                              Cancel
                            </button>
                          )}
                  ...
                  <>
                      ...
                      <div style={{ marginBottom: "8px" }}>
                        ...
                        >
                          ...
                          >
                            <input
                              type="radio"
                              name={`${ch.id}_connection_mode`}
                              value="websocket"
                              checked={connMode === "websocket"}
                              onChange={() =>
                                setConnectionModes((prev) => ({
                                  ...prev,
                                  [ch.id]: "websocket",
                    ...
                    {/* Form fields */}
                    {formFields.map((field) =>
                      renderField(field, ch.id, form[field.key] || "", (val) =>
                        setFormField(ch.id, field.key, val),
                      ),
                    )}
                    ...
                    {ch.id === "atlassian" && (
                      <>
                        ...
                        >
                          Service account key starts with <code>ATSTT</code>.
                          Personal API token: base64-encode{" "}
                          <code>email:token</code> and prefix with{" "}
                          <code>Basic </code>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          Required for multi-site setups. Find it at{" "}
                          <code>your-site.atlassian.net/_edge/tenant_info</code>
                        </div>
                      </>
                    )}
                    ...
                    >
                      ...
                      >
                        {saveMutation.isPending
                          ...
                          : isEditing
                            ? "Save Changes"
                            : t("agent.settings.channel.saveChannel")}
                      </button>
                      {isEditing && (
                        <button
                          className="btn btn-secondary"
