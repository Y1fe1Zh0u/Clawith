...
export default function TakeControlPanel({
  agentId,
  sessionId,
  onClose,
  envType = "browser",
  ...
  const [statusText, setStatusText] = useState(
    t("takeControl.acquiringControl"),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mountedRef.current) {
          ...
          try {
            const urlRes = await controlApi.currentUrl(agentId, {
              session_id: sessionId,
            });
            if (urlRes.url) {
              const hostname = new URL(urlRes.url).hostname.replace(
                /^www\./,
                "",
              );
    ...
    return () => {
      ...
      if (lockedRef.current) {
        controlApi
          .unlock(agentId, { session_id: sessionId, export_cookies: false })
          .catch(() => {});
      }
    };
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (cancelled) return;
      try {
        const res = await controlApi.screenshot(agentId, {
          session_id: sessionId,
        });
  ...
  const handleMouseUp = useCallback(
    async (e: React.MouseEvent<HTMLImageElement>) => {
      ...
      if (isDraggingRef.current) {
        ...
        try {
          const res = await controlApi.click(agentId, {
            session_id: sessionId,
            x: coords.x,
            y: coords.y,
          });
  ...
  const handleSendText = useCallback(async () => {
    ...
    try {
      const res = await controlApi.type(agentId, {
        session_id: sessionId,
        text: textInput,
      });
  ...
  const handleQuickKey = useCallback(
    async (keys: string[]) => {
      ...
      try {
        const res = await controlApi.pressKeys(agentId, {
          session_id: sessionId,
          keys,
        });
        if (res.status === "error")
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      try {
        const finalRes = await controlApi.screenshot(agentId, {
          session_id: sessionId,
        });
      ...
      if (lastScreenshotRef.current && onLastScreenshot) {
        console.log(
          "[TakeControl] Passing last screenshot to parent on complete, size:",
          lastScreenshotRef.current.length,
        );
        onLastScreenshot(lastScreenshotRef.current);
      } else {
        console.log(
          "[TakeControl] No screenshot to pass: ref=",
  ...
  const handleCancel = useCallback(async () => {
    if (!locked) {
      ...
      if (lastScreenshotRef.current && onLastScreenshot) {
        console.log(
          "[TakeControl] Passing last screenshot to parent on cancel (unlocked), size:",
          lastScreenshotRef.current.length,
        );
        onLastScreenshot(lastScreenshotRef.current);
      } else {
        console.log(
          "[TakeControl] No screenshot to pass on cancel (unlocked): ref=",
          !!lastScreenshotRef.current,
          "callback=",
          !!onLastScreenshot,
        );
    ...
    if (lastScreenshotRef.current && onLastScreenshot) {
      console.log(
        "[TakeControl] Passing last screenshot to parent on cancel (locked), size:",
        lastScreenshotRef.current.length,
      );
      onLastScreenshot(lastScreenshotRef.current);
    } else {
      console.log(
        "[TakeControl] No screenshot to pass on cancel (locked): ref=",
        !!lastScreenshotRef.current,
        "callback=",
        !!onLastScreenshot,
      );
      ...
      <div className="tc-panel">
        ...
        <div className="tc-header">
          <div className="tc-header-left">
            ...
            <span className="tc-status" key={statusFlashKey}>
              {statusText}
            </span>
          </div>
          <button
            ...
            title={t("takeControl.exitWithoutSaving")}
          >
            {CloseIcon}
          </button>
        </div>
          ...
          {screenshot ? (
            ...
            >
              <img
                ...
                style={{
                  cursor: locked ? "crosshair" : "default",
                  userSelect: "none",
                  display: "block",
                  maxWidth: "100%",
                  maxHeight: "100%",
        ...
        <div className="tc-action-bar">
          <div className="tc-domain-row">
            <span className="tc-domain-label">
              {t("takeControl.saveLoginStateTo")}
            </span>
            <input
