...
export function CreatorLine({ entry }: { entry: ExperienceEntry }) {
  ...
  return (
    ...
    >
      ...
      {hasCreators && created && <span style={{ opacity: 0.5 }}>·</span>}
...
export function EntryDrawer({
  ...
  const {
    ...
    queryKey: ["experience-entry", entryId],
    queryFn: () => experienceApi.get(entryId),
    retry: false,
  });
  ...
  const retire = useMutation({
    mutationFn: () => experienceApi.retire(entryId),
    onSuccess: () => {
      onChanged();
      onClose();
    },
  ...
  const republish = useMutation({
    mutationFn: () => experienceApi.publish(entryId),
    onSuccess: () => {
      onChanged();
      onClose();
    },
  ...
  const review = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["experience-entry", entryId] });
      onChanged();
    },
  });
  ...
  return (
    <Drawer onClose={onClose} docked={docked}>
      ...
      {refs && (
        ...
        >
          被 AI 阅读 {refs.read_count} 次 ·{" "}
          <strong style={{ color: "var(--text-secondary)" }}>
            实际采纳（引用） {refs.cited_count} 次
          </strong>
        </div>
      )}
      {entry.can_manage ? (
        <>
          ...
          >
            <button onClick={() => onEdit(entry)} style={secondaryBtn}>
              编辑
            </button>
            ...
            {entry.status === "retired" ? (
              <button
                ...
                style={{ ...secondaryBtn, color: "var(--success)" }}
