...
export default function FileBrowser({
    ...
    try {
      ...
      if (fileFilter && fileFilter.length > 0) {
        data = data.filter(
          (f) =>
            f.is_dir ||
            fileFilter.some((ext) => f.name.toLowerCase().endsWith(ext)),
        );
      }
      setFiles(data);
    } catch (error) {
  ...
  const { isDragging, dropZoneProps } = useDropZone({
    ...
    onReject: (files) =>
      showToast(
        t("agent.upload.unsupported", "Unsupported file type: {{name}}", {
          name: files[0]?.name || "",
        }),
        "error",
      ),
  ...
  const renderBreadcrumbs = () => {
    ...
    return (
      ...
      >
        ...
        {pathParts
          .slice(rootPath ? rootPath.split("/").filter(Boolean).length : 0)
          .map((part, i) => {
            const upTo = pathParts
              .slice(
                0,
        ...
        >
          ...
          {isText &&
            ...
            (!editing ? (
              ...
              >
                <IconEdit size={13} stroke={1.8} /> {t("agent.soul.editButton")}
              </button>
            ) : (
        ...
        <div style={{ display: "flex", gap: "6px", marginLeft: "auto" }}>
          {upload && api.upload && (
            ...
            >
              ...
              {t("agent.workspace.uploadFile", "Upload File")}
            </button>
          )}
          {newFolder && (
            <button
              ...
              onClick={() =>
                setPromptModal({
                  title: t("agent.workspace.newFolder"),
                  placeholder: t("agent.workspace.newFolderName"),
                  action: "newFolder",
                })
              }
            >
          ...
          {newFile && !fileFilter && (
            <button
              ...
              onClick={() =>
                setPromptModal({
                  title: t("agent.workspace.newFile", "New File"),
            ...
            >
              ...
              >
                ...
                {!f.is_dir && api.downloadUrl && (
                  <a
                    href={api.downloadUrl(f.path || `${currentPath}/${f.name}`)}
                    download
