...
function isWorkspacePath(path?: string | null): boolean {
  return (
    !!path && (path === WORKSPACE_ROOT || path.startsWith(`${WORKSPACE_ROOT}/`))
  );
}
...
function isEnterprisePath(path?: string | null): boolean {
  return (
    !!path &&
    (path === ENTERPRISE_ROOT || path.startsWith(`${ENTERPRISE_ROOT}/`))
  );
}
...
function buildRevisionDiff(revision: WorkspaceRevision): string {
  ...
  if (before === after) {
    ...
    if (revision.operation === "autosave")
      return "Autosaved with no textual changes.";
...
export default function WorkspaceOperationPanel({
  ...
  const canEdit =
    !!activePath && EDITABLE_EXTS.has(ext) && canModifyPath(activePath);
  const isHtml = ext === ".html" || ext === ".htm";
  const isImage = IMAGE_EXTS.has(ext);
  const activityKey = activities
    .map((item) => `${item.action}:${item.path}`)
    .join("|");
  const treeTargetDir = normalizeWritableDir(
    canModifyPath(selectedDirPath) ? selectedDirPath : directoryOf(activePath),
  );
  const panelSideWidth = activityOpen
    ? Math.max(sideWidth, DEFAULT_HISTORY_WIDTH)
    : sideWidth;
  const draftMatchesActiveFile = !!(
    liveDraft?.path &&
    activePath &&
    liveDraft.path === activePath
  );
  const shouldRenderLiveDraft =
  ...
  const loadFileTree = useCallback(async () => {
    const loadDir = async (
      ...
      return Promise.all(
        items.map(async (item: WorkspaceFileNode) => {
    ...
    const roots = await loadDir(
      treeScope === "workspace" ? WORKSPACE_ROOT : "",
      0,
    );
    setFileTree(roots);
  ...
  useEffect(() => {
    ...
    if (
      liveDraft &&
      (!activePath || !liveDraft.path || liveDraft.path === activePath)
    ) {
      setPreview(null);
  ...
  useEffect(() => {
    ...
    if (
      latestActivity?.action !== "delete" ||
      latestActivity.ok === false ||
      latestActivity.pendingApproval
    )
      return;
    setEditing(false);
    onEditingChange?.(false);
  ...
  useEffect(() => {
    ...
    const latestActivity = activities.find((item) => item.path === activePath);
    if (latestActivity?.action !== "delete" || !latestActivity.pendingApproval)
      return undefined;
  ...
  const csvRows = useMemo(() => {
    if (previewType === "csv")
      return parseCsv(editing ? draft : content)
        .slice(0, 200)
  ...
  const renderPreview = () => {
      ...
      if (draftExt === ".html" || draftExt === ".htm") {
        return (
          <div className="workspace-op-live">
            <div className="workspace-op-live-banner">
              {liveDraft.status === "drafting"
                ? "Drafting HTML..."
                : "Writing HTML..."}
            </div>
            {draftContent ? (
              <HtmlPreviewFrame
                ...
                title={fileName(liveDraft.path || "draft.html")}
                suspendAutoFit={isSideResizing}
              />
            ) : (
              <div className="workspace-op-empty">
    ...
    if (!activePath) {
      return (
        <div className="workspace-op-empty">
          No workspace file activity yet.
        </div>
      );
    }
    ...
    if (previewState === "deleted") {
      return (
        <div className="workspace-op-empty workspace-op-deleted">
          <strong className="workspace-op-deleted-title">
    ...
    if (previewType === "text") {
      return (
        <pre className="workspace-op-text-preview">
    ...
    if (isHtml) {
      if (isSideResizing) {
        return (
          <div className="workspace-op-empty workspace-op-preview-paused">
            Release to refresh HTML preview.
          </div>
        );
      }
      return (
    ...
    if (previewType === "pdf") {
      if (isSideResizing) {
        return (
          <div className="workspace-op-empty workspace-op-preview-paused">
            Release to refresh PDF preview.
          </div>
        );
      }
      return (
        <iframe
          className="workspace-op-pdf"
          src={fileApi.downloadUrl(agentId, activePath, { inline: true })}
          title={fileName(activePath)}
        />
      );
    ...
    return (
      <div className="workspace-op-empty">
        Preview is not available for this file. Download it instead.
      </div>
    );
  };

  const renderUploadRows = (dirPath: string, depth: number) =>
    uploadItems
      ...
      .map((item) => (
        ...
        >
          ...
          {item.status === "error" && (
            <div className="workspace-op-tree-upload-error">{item.error}</div>
          )}
        </div>
      ));

  const renderFileTreeNodes = (nodes: WorkspaceFileNode[], depth = 0) =>
    nodes.map((node) => {
      ...
      if (node.is_dir) {
        ...
        return (
          <div key={node.path || node.name}>
            <div
              className={`workspace-op-tree-dir ${dirSelected ? "active" : ""}`}
            ...
            >
              ...
              {node.path !== WORKSPACE_ROOT &&
                node.path !== SKILLS_ROOT &&
                node.path !== MEMORY_ROOT &&
                node.path !== ENTERPRISE_ROOT &&
                canModifyPath(node.path) && (
                  <button
  ...
  const openInNewTabLabel = t(
    "agent.workspace.openInNewTab",
    "Open in new tab",
  );
      ...
      {canManageWorkspace && (
        <>
          ...
          >
            ...
            >
              <path
                d="M4 8.5A2.5 2.5 0 016.5 6H10l1.4 1.6H17.5A2.5 2.5 0 0120 10.1v6.4A2.5 2.5 0 0117.5 19h-11A2.5 2.5 0 014 16.5v-8Z"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinejoin="round"
              />
  ...
  const headerActions = (
    <>
      {saveState !== "idle" && (
        <span className={`workspace-op-save ${saveState}`}>{saveState}</span>
      )}
      {activePath && isHtml && !shouldRenderLiveDraft && (
        <a
          ...
          href={
            htmlPreviewSrc ||
            fileApi.downloadUrl(agentId, activePath, { inline: true })
          }
          target="_blank"
          rel="noreferrer"
        ...
        >
          ...
          >
            ...
            />
            <path
              d="M11 5H7a3 3 0 00-3 3v9a3 3 0 003 3h9a3 3 0 003-3v-4"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
              strokeLinejoin="round"
      ...
      {activePath && onToggleLock && (
        ...
        >
          ...
          >
            <path
              d="M4 9V6.5A2.5 2.5 0 016.5 4H9"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M15 4h2.5A2.5 2.5 0 0120 6.5V9"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M20 15v2.5a2.5 2.5 0 01-2.5 2.5H15"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M9 20H6.5A2.5 2.5 0 014 17.5V15"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
              strokeLinejoin="round"
  ...
  const hasHeaderActions =
    saveState !== "idle" ||
    !!(activePath && (canEdit || onToggleLock || isHtml));
  ...
  return (
    <div className="workspace-op">
      ...
      >
        {!treeOpen && !activityOpen && (
          <button
            className="workspace-op-tree-edge-toggle"
            onClick={() => {
              setTreeOpen(true);
            }}
            title="Show files"
        ...
        {(treeOpen || activityOpen) && (
          <div className="workspace-op-side-resize" onMouseDown={startResize} />
        )}
        {activityOpen ? (
          <aside className="workspace-op-side">
            ...
            <div className="workspace-op-side-list">
              {!activePath && (
                <div className="workspace-op-side-empty">
                  Open a file to view its history.
                </div>
              )}
              {activePath && revisions.length === 0 && (
                <div className="workspace-op-side-empty">
              ...
              {activePath &&
                revisions.map((rev) => {
                  ...
                  const isNote =
                    diffText.startsWith("No ") ||
                    diffText.startsWith("Restored") ||
                    diffText.startsWith("Autosaved");
                  return (
                    <div className="workspace-op-revision" key={rev.id}>
                      <div className="workspace-op-revision-head">
                        ...
                        <time
                          className="workspace-op-revision-time"
                          dateTime={rev.created_at || undefined}
                        >
                          {formatRevisionTime(rev.created_at)}
                        </time>
                      </div>
                      <pre
                        className={isNote ? "workspace-op-revision-note" : ""}
                      >
                        {diffText}
                      </pre>
                      {rev.after_content != null && (
                        <button
          ...
          <aside className="workspace-op-tree">
            <div className="workspace-op-side-title">
              <div className="workspace-op-tree-tools workspace-op-tree-tools-full">
                <div
                  className="workspace-op-tree-scope"
                  role="tablist"
                  aria-label="File tree scope"
                >
                  <button
            ...
            <div className="workspace-op-tree-list">
              {treeScope === "workspace" &&
                renderUploadRows(WORKSPACE_ROOT, -1)}
              {fileTree.length ? (
                renderFileTreeNodes(fileTree, 0)
              ) : (
                <div className="workspace-op-tree-empty">No files yet.</div>
              )}
            </div>
            <input
              ref={fileInputRef}
      ...
      {pendingSwitchPath && (
        <div
          className="workspace-op-modal-overlay"
          onClick={() => setPendingSwitchPath(null)}
        >
          <div
            className="workspace-op-modal"
          ...
          >
            ...
            <div className="workspace-op-modal-text">
              You are editing{" "}
              <strong>
                {activePath ? fileName(activePath) : "the current file"}
              </strong>
              . Choose how to handle your changes before opening{" "}
              <strong>{fileName(pendingSwitchPath)}</strong>.
            </div>
            <div className="workspace-op-modal-actions">
              ...
              <button className="btn btn-secondary" onClick={discardAndSwitch}>
                Discard & Switch
              </button>
              <button className="btn btn-primary" onClick={saveAndSwitch}>
                Save & Switch
              </button>
            </div>
          </div>
