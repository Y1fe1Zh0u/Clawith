...
const HairlineInput = forwardRef<HTMLInputElement, Props>(
  function HairlineInput({ label, serif, className, id, ...rest }, ref) {
    const inputId =
      id ||
      (label
        ? `atlas-input-${label.replace(/\s+/g, "-").toLowerCase()}`
        : undefined);
    const inputClasses = [
      "atlas-input",
