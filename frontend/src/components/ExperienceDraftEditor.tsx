...
export function DraftEditor({
  ...
  const footer = (
    <>
      {err && (
        <div
          style={{ color: "var(--error)", fontSize: 13, margin: "0 0 10px" }}
        >
          {err}
        </div>
      )}
  ...
  return (
    <Drawer onClose={onClose} docked={docked} header={header} footer={footer}>
      ...
      >
        {bodyLen} 字
        {bodyLen > BODY_SOFT_LIMIT &&
          ` · 偏长，AI 每次读取都要吃掉全文，建议精简到 ${BODY_SOFT_LIMIT} 字内`}
      </div>
      ...
      >
        AI
        检索时只先看到标题和这一栏，据此判断要不要读全文——请写成脱离正文也能读懂的一两句话。
      </p>
