...
const TYPE_META: Record<ToastType, { color: string; icon: ReactNode }> = {
  ...
  success: {
    color: "var(--success)",
    icon: <IconCheck size={12} stroke={2.4} />,
  },
  warning: {
    color: "var(--warning)",
    icon: <IconAlertTriangle size={12} stroke={2} />,
  },
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const show = useCallback(
    (type: ToastType, message: string, options: ToastOptions = {}) => {
      ...
      setItems((list) => [
        ...list,
        { id, type, message, details: options.details, duration },
      ]);
    },
    [],
  );
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {item.details && (
          <>
            ...
            >
              {showDetails
                ? t("common.toast.collapseDetails")
                : t("common.toast.viewDetails")}
            </button>
            {showDetails && (
              <pre
                style={{
