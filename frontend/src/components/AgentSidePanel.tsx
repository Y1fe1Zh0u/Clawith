...
import WorkspaceOperationPanel, {
  WorkspaceActivity,
  WorkspaceLiveDraft,
} from "./WorkspaceOperationPanel";
...
export type SidePanelTab =
  "workspace" | "aware" | "browser" | "desktop" | "code" | "transfer";
...
export default function AgentSidePanel({
  ...
  useEffect(() => {
    const onResize = () => {
      if (!isDragging.current && !userResized.current)
        setPanelWidth(calcInitialWidth());
    };
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      setPanelWidth(
        Math.min(maxWidth, Math.max(MIN_WIDTH, dragStartWidth.current + delta)),
      );
    };
    const onMouseUp = () => {
      if (!isDragging.current) return;
  ...
  return (
    <div
      className="live-panel agent-side-panel"
    ...
    >
      ...
      <div className="live-panel-header">
        {availableTabs.length > 1 ? (
          <div
            className="agent-side-panel-tabs"
            role="tablist"
            aria-label={t("agent.sidePanel.previewTabs", "Preview areas")}
          >
        ...
        <div className="live-panel-header-right">
          ...
          {activeTab === "workspace" && (
            <>
              <div
                id="workspace-header-file-actions"
                className="live-panel-workspace-actions"
              />
              <button
                ...
                aria-label={t(
                  "agent.workspace.versionHistory",
                  "Version history",
                )}
              >
                <svg
          ...
          <div className="live-panel-transfer">
            <div className="live-panel-transfer-card">
              <div className="live-panel-transfer-title">
                ...
                <span>
                  {liveState.transfer.status === "running"
                    ? t(
                        "agent.sidePanel.transferRunning",
                        "Transfer in progress",
                      )
                    : t("agent.sidePanel.transferDone", "Transfer complete")}
                </span>
              </div>
              <div className="live-panel-transfer-route">
                <div>
                  ...
                  {liveState.transfer.fromPath && (
                    <code>{liveState.transfer.fromPath}</code>
                  )}
                </div>
                ...
                <div>
                  ...
                  {liveState.transfer.toPath && (
                    <code>{liveState.transfer.toPath}</code>
                  )}
                </div>
              </div>
              {liveState.transfer.result && (
                <pre className="live-panel-transfer-result">
                  {liveState.transfer.result}
                </pre>
              )}
            </div>
