...
const Icons = {
  ...
  fire: sicon(
    "M8.5 1.5S12.5 5 12.5 9a4.5 4.5 0 01-9 0c0-2 1-3.5 2-4.5 0 0 .5 2 2 2.5C8 7 8.5 1.5 8.5 1.5z",
  ),
  check: sicon("M13.5 4.5l-7 7-3-3"),
  trophy: sicon(
    "M5 14h6M8 11v3M4 2h8v3a4 4 0 01-8 0V2zM4 3H2.5a1 1 0 00-1 1v1a2 2 0 002 2H4M12 3h1.5a1 1 0 011 1v1a2 2 0 01-2 2H12",
  ),
...
export default function Plaza() {
  ...
  const teamQ = useQuery({
    queryKey: ["experience", "team"],
    queryFn: () => experienceApi.list({ view: "team" }),
    enabled: view === "team",
  });
  const taggedTeamQ = useQuery({
    queryKey: ["experience", "team", "tag", teamTag],
    queryFn: () =>
      experienceApi.list({ view: "team", tag: teamTag || undefined }),
    enabled: view === "team" && !!teamTag,
  });
  const statsQ = useQuery({
    queryKey: ["experience-stats"],
    queryFn: () => experienceApi.stats(),
    enabled: view === "team",
  });
  const mineQ = useQuery({
    queryKey: ["experience", "mine"],
    queryFn: () => experienceApi.list({ view: "mine" }),
    enabled: view === "mine",
  });
  ...
  const trending = useMemo(() => {
    ...
    unfilteredTeamEntries.forEach((e) =>
      (e.tags || []).forEach((tg) => m.set(tg, (m.get(tg) || 0) + 1)),
    );
  ...
  const mineShown =
    cat === MINE_DRAFTS
      ? mineEntries.filter((e) => e.status === "draft")
      : cat === MINE_RETIRED
        ? mineEntries.filter((e) => e.status === "retired")
        : cat === MINE_UNTAGGED
          ? mineEntries.filter(
              (e) => e.status !== "retired" && !(e.tags || []).length,
            )
          : cat === MINE_ALL
            ? mineEntries.filter((e) => e.status !== "retired")
            : mineEntries.filter(
                (e) => e.status !== "retired" && (e.tags || []).includes(cat),
              );
    ...
    <div style={{ padding: 24, maxWidth: 1120, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          >
            {view === "team"
              ? t("experience.feedTitle", "经验广场")
              : t("experience.nav.mine", "我的经验")}
          </h1>
          <p
            style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <SegmentedToggle
            ...
            options={[
              { value: "team", label: t("experience.nav.team", "团队经验") },
              { value: "mine", label: t("experience.nav.mine", "我的经验") },
            ]}
          />
          <button
      ...
      {view === "team" ? (
        <TeamView
          loading={teamQ.isLoading || taggedTeamQ.isLoading}
          entries={teamEntries}
          stats={statsQ.data}
          trending={trending}
          activeTag={teamTag}
...
function SegmentedToggle({
  ...
  value: "team" | "mine";
  onChange: (v: "team" | "mine") => void;
  options: { value: "team" | "mine"; label: string }[];
}) {
  return (
    <div
      style={{
      ...
      {options.map(({ value: v, label }) => (
        <button
          ...
          style={{
            ...
            transition: "all .15s",
            background:
              value === v ? "var(--segment-active-bg)" : "transparent",
            color:
              value === v
...
function StatsBar({
  ...
  stats?: { total: number; today: number; cited: number };
}) {
  const { t } = useTranslation();
  const items = [
    {
      icon: Icons.book,
      label: t("experience.stat.total", "经验数"),
      value: stats?.total ?? 0,
    },
    {
      icon: Icons.fire,
      label: t("experience.stat.today", "今日新增"),
      value: stats?.today ?? 0,
    },
    {
      icon: Icons.check,
      label: t("experience.stat.cited", "被采纳"),
      value: stats?.cited ?? 0,
    },
  ];
  return (
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
          ...
          {loading ? (
            ...
            activeTag ? (
              ...
              >
                ...
                <button className="btn btn-secondary" onClick={onClearTag}>
                  {t("experience.clearFilter", "清除筛选")}
                </button>
              </div>
            ) : (
              <div
              ...
              >
                ...
                >
                  {t(
                    ...
                    "在与数字员工的对话里点「沉淀为经验」，或直接新建，把团队踩过的坑沉淀下来——AI 会在相关任务中自动检索复用。",
                  )}
                </div>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              {entries.map((e) => (
                <EntryCard key={e.id} entry={e} onOpen={() => onOpen(e)} />
              ))}
            </div>
          )}
        </div>
        ...
        >
          {stats && stats.top_contributors.length > 0 && (
            <SidebarSection
              icon={Icons.trophy}
              title={t("experience.topContributors", "热门贡献者")}
            >
          ...
          {trending.length > 0 && (
            <SidebarSection
              icon={Icons.hash}
              title={t("experience.trendingTags", "热门标签")}
            >
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: "var(--text-xs)",
                        background: on
                          ? "var(--accent-soft, var(--accent, var(--bg-tertiary)))"
                          : "var(--bg-tertiary)",
                        color: on
          ...
          {(!stats || stats.top_contributors.length === 0) &&
            trending.length === 0 && (
              ...
              >
                {t(
                  "experience.sidebarEmpty",
                  "有了已发布经验后，这里会显示热门贡献者与标签。",
                )}
              </div>
            )}
        </aside>
        ...
        {cats.map((c) => (
          <button
            ...
            style={{
              ...
              border:
                "1px solid " +
                (cat === c ? "transparent" : "var(--border-default)"),
              background: cat === c ? "var(--accent-primary)" : "transparent",
              color:
                cat === c ? "var(--text-inverse)" : "var(--text-secondary)",
            }}
          >
            {c}
          </button>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {entry.status !== "published" && (
            <Badge tone="warn">
              {entry.status === "retired" ? "已下架" : "草稿"}
            </Badge>
          )}
          {daysLeft !== null && (
            <Badge tone="warn">{daysLeft} 天后自动删除</Badge>
          )}
