...
export default function Login() {
  ...
  const [verificationEntryMode, setVerificationEntryMode] = useState<
    "create" | "join" | "home"
  >("home");
    ...
    try {
      if (isRegister) {
        ...
        if (regRes.user?.email_verified || regRes.user?.is_active) {
          if (invitationCode) {
            ...
            navigate("/setup-company", {
              state: {
                fromRegister: true,
                email: regRes.email || form.login_identifier,
              },
            });
          } else {
            navigate("/");
        ...
        enterVerificationStep(
          regRes.email || form.login_identifier,
        ...
        const res = await authApi.login({
          ...
          ...(tenant?.id &&
          tenant.sso_domain &&
          !tenant.sso_domain.match(/^https?:\/\/\d{1,3}(\.\d{1,3}){3}(:\d+)?$/)
            ? { tenant_id: tenant.id }
            : {}),
        });
        ...
        if (
          "requires_tenant_selection" in res &&
          res.requires_tenant_selection
        ) {
      ...
      if (
        ...
        if (msg.includes("company has been disabled")) {
          ...
          setError(
            t("auth.emailAlreadyRegistered", "该邮箱已注册，请直接登录"),
          );
        } else {
          setError(msg);
        }
  ...
  const shouldShowGlobalOAuth =
    !tenant?.sso_enabled && !isRegister && !showVerification;
      ...
      <div className="atlas-screen-split atlas-login-split">
        ...
        <div className="atlas-screen-plate atlas-login-hero">
          ...
          <div className="atlas-login-welcome">
            ...
            <p className="atlas-body atlas-body--muted">
                    ...
                    <div style={{ marginBottom: "24px" }}>
                      ...
                      >
                        ...
                        >
                          {t(
                            "auth.ssoNotice",
                            "Enterprise SSO is enabled for this domain.",
                          )}
                        </div>
                      </div>
                  ...
                  <form onSubmit={handleSubmit} className="login-form">
                    <div className="login-field">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm({ ...form, login_identifier: e.target.value })
                        }
                        required
                        autoFocus
                    ...
                    >
                        ...
                        {tenantSelection.map((tenant) => (
                          <button
                            ...
                            onMouseEnter={(e) => {
                              ...
                              (
                                e.currentTarget as HTMLButtonElement
                              ).style.borderColor = "rgba(17,17,20,0.2)";
                            }}
                            onMouseLeave={(e) => {
                              (
                                e.currentTarget as HTMLButtonElement
                              ...
                              (
                                e.currentTarget as HTMLButtonElement
                              ).style.borderColor = "rgba(17,17,20,0.1)";
                            }}
                          >
                            {tenant.tenant_name}{" "}
                            {tenant.tenant_slug && `(${tenant.tenant_slug})`}
                          </button>
                        ))}
                      </div>
                      ...
                      <button
                        ...
                        onMouseEnter={(e) => {
                          (
                            e.currentTarget as HTMLButtonElement
                          ).style.borderColor = "rgba(17,17,20,0.32)";
                        ...
                        onMouseLeave={(e) => {
                          (
                            e.currentTarget as HTMLButtonElement
                          ).style.borderColor = "rgba(17,17,20,0.18)";
                          (e.currentTarget as HTMLButtonElement).style.color =
                            "#8c8c96";
                        }}
                      >
                        {t(
                          "auth.createOrJoinOrganization",
                          "Create or Join Organization",
                        )}
                      </button>
                      <button
