export type ToolResolutionStatus =
  "checking" | "saved" | "not_saved" | "partial" | "conflicted" | "unavailable";
  ...
  if (incoming.toolCallId) {
    const exactIndex = messages.findIndex(
      (message) =>
        ...
        message.toolCallId === incoming.toolCallId,
    );
    if (exactIndex >= 0) {
      const existing = messages[exactIndex];
      if (existing.toolStatus === "done" && incoming.toolStatus === "running")
        return messages;
      return [
...
export const reduceSessionStreamChunk = (
  ...
  if (previousSequence !== undefined) {
    if (sequence <= previousSequence || sequence !== previousSequence + 1)
      return current;
  }

  return {
    content:
      packet.reset === true || sequence === 1
        ? content
...
export const mergeInterruptedStreamMessage = <
...
): Map<string, ToolReconciliation> =>
  new Map(
    reconciliations.map((reconciliation) => [
      reconciliation.toolCallId,
      reconciliation,
    ]),
  );

export const sessionActiveRunFromResponse = (
  payload: unknown,
): SessionActiveRun | null => {
  ...
  if (rawReconciliations != null && !Array.isArray(rawReconciliations))
    return null;
  ...
  for (const value of rawReconciliations || []) {
    ...
    const rawResolutionStatus = firstDefined(
      item,
      "resolution_status",
      "resolutionStatus",
    );
    if (
      ...
      (typeof rawResolutionStatus !== "string" ||
        !TOOL_RESOLUTION_STATUSES.has(
          rawResolutionStatus as ToolResolutionStatus,
        ))
    )
      return null;
    pendingToolReconciliations.push({
      ...
      ...(rawResolutionStatus !== undefined
        ...
        : {}),
      ...(optionalCount(firstDefined(item, "saved_count", "savedCount")) !==
      undefined
        ? {
            savedCount: optionalCount(
              firstDefined(item, "saved_count", "savedCount"),
            ),
          }
        : {}),
      ...(optionalCount(firstDefined(item, "pending_count", "pendingCount")) !==
      undefined
        ? {
            pendingCount: optionalCount(
              firstDefined(item, "pending_count", "pendingCount"),
            ),
          }
        : {}),
      ...(optionalCount(
        firstDefined(item, "conflicted_count", "conflictedCount"),
      ) !== undefined
        ? {
            conflictedCount: optionalCount(
              firstDefined(item, "conflicted_count", "conflictedCount"),
            ),
          }
        : {}),
      ...(typeof firstDefined(
        ...
        ? {
            workspaceResolution: firstDefined(
              item,
              "workspace_resolution",
              "workspaceResolution",
            ) as boolean,
...
export const failClosedSessionActiveRun = (
  ...
  current
    ? {
        ...
        canCancel: false,
        pendingToolReconciliations: (
          current.pendingToolReconciliations || []
        ).map((item) => ({
          ...item,
          canReconcile: false,
        })),
...
export const mergeTerminalAssistantMessage = <
  T extends TerminalAssistantMessageLike,
>(
  messages: T[],
  terminalMessage: T,
): T[] => {
