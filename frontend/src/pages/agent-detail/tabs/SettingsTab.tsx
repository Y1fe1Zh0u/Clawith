  ...
  return (
    ...
    >
      <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            {settingsError && (
              <span
                style={{
                  fontSize: "12px",
                  color: settingsError.includes("adjusted")
                    ? "var(--warning)"
                    : "var(--error)",
                  whiteSpace: "pre-line",
                }}
            ...
            {canManage && (
              ...
              >
                {settingsSaving
                  ? t("agent.settings.saving", "Saving...")
                  : t("agent.settings.save", "Save")}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          ...
          >
            <div>
              ...
              {settingsForm.primary_model_id &&
                llmModels.some(
                  ...
                  >
                    {t(
                      "agent.settings.modelDisabledWarning",
                      "This model has been disabled by admin. The agent will automatically use the fallback model.",
                    )}
                  </div>
                )}
              <div
            ...
            <div>
              ...
              >
                {t("agent.settings.fallbackModel")}
              </label>
              <select
                className="input"
                ...
                onChange={(e) =>
                  setSettingsForm((form) => ({
                    ...form,
                    fallback_model_id: e.target.value,
                  }))
                }
              >
                <option value="">--</option>
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          <h4 style={{ marginBottom: "12px" }}>
            {t("agent.settings.conversationContext")}
          </h4>
          <div>
            <label
              style={{
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          ...
          <div>
            ...
            >
              {t(
                "agent.settings.maxToolRoundsLabel",
                "Maximum model steps per run",
              )}
            </label>
            <input
              className="input"
            ...
            >
              {t(
                "agent.settings.maxToolRoundsDesc",
                "Each model step that returns a result counts, including wait or error results. Tool-call count is separate. Default: 50",
              )}
            </div>
          </div>
        </div>
          ...
          >
            <div>
              ...
              >
                ...
                {formatTokens(agent?.tokens_used_today || 0)}
              </div>
            </div>
            <div>
              <label
              ...
              <input
                ...
                onChange={(e) =>
                  setSettingsForm((form) => ({
                    ...form,
                    max_tokens_per_month: e.target.value,
                  }))
                }
                placeholder={t("agent.settings.noLimit")}
              />
              ...
              >
                ...
                {formatTokens(agent?.tokens_used_month || 0)}
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div>
                  ...
                  >
                    {isChinese
                      ? "Agent 最多可同时拥有的触发器数量"
                      : "Max active triggers the agent can have"}
                  </div>
                </div>
                <div>
                  <label
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettingsForm((form) => ({
                        ...form,
                        min_poll_interval_min: Math.max(
                          1,
                  ...
                  >
                    {isChinese
                      ? "定时轮询外部接口的最短间隔"
                      : "Minimum interval for polling external URLs"}
                  </div>
                </div>
                <div>
                  <label
                  ...
                  >
                    {isChinese
                      ? "外部系统每分钟最多可调用的 Webhook 次数"
                      : "Max webhook calls per minute from external services"}
                  </div>
                </div>
              </div>
            </div>
          ...
          return (
            <div className="card" style={{ marginBottom: "12px" }}>
              ...
              <textarea
                ...
                onBlur={() => {
                  if (canManage) void onSaveWelcomeMessage();
                }}
                placeholder={
                  isChinese
                    ? "例如：你好！我是你的 AI 助手，有什么可以帮你的吗？"
                    : "e.g. Hello! I'm your AI assistant. How can I help you?"
                }
                style={{
                  width: "100%",
                  minHeight: "80px",
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          <h4 style={{ marginBottom: "4px" }}>
            {t("agent.settings.autonomy.title")}
          </h4>
          <p
            style={{
              fontSize: "12px",
          ...
          >
            {[
              {
                key: "read_files",
                label: t("agent.settings.autonomy.readFiles"),
                desc: t("agent.settings.autonomy.readFilesDesc"),
              },
              {
                key: "write_workspace_files",
                label: t("agent.settings.autonomy.writeFiles"),
                desc: t("agent.settings.autonomy.writeFilesDesc"),
              },
              {
                key: "delete_files",
                label: t("agent.settings.autonomy.deleteFiles"),
                desc: t("agent.settings.autonomy.deleteFilesDesc"),
              },
              {
                key: "send_feishu_message",
                label: t("agent.settings.autonomy.sendFeishu"),
                desc: t("agent.settings.autonomy.sendFeishuDesc"),
              },
              {
                key: "web_search",
                label: t("agent.settings.autonomy.webSearch"),
                desc: t("agent.settings.autonomy.webSearchDesc"),
              },
              {
                key: "manage_tasks",
                label: t("agent.settings.autonomy.manageTasks"),
              ...
              return (
                ...
                >
                  ...
                  >
                    <option value="L1">
                      {t("agent.settings.autonomy.l1Auto")}
                    </option>
                    <option value="L2">
                      {t("agent.settings.autonomy.l2Notify")}
                    </option>
                    <option value="L3">
                      {t("agent.settings.autonomy.l3Approve")}
                    </option>
                  </select>
                </div>
              );
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          ...
          >
            {t(
              "agent.settings.timezone.description",
              "The timezone used for this agent's scheduling, active hours, and time awareness. Defaults to the company timezone if not set.",
            )}
          </p>
          <div
            style={{
          ...
          >
            <div>
              <div style={{ fontWeight: 500, fontSize: "13px" }}>
                {t("agent.settings.timezone.current", "Agent Timezone")}
              </div>
              <div style={{ fontSize: "11px", color: "var(--text-tertiary)" }}>
                {agent?.timezone
                  ? t(
                      "agent.settings.timezone.override",
                      "Custom timezone for this agent",
                    )
                  : t(
                      "agent.settings.timezone.inherited",
                      "Using company default timezone",
                    )}
              </div>
            </div>
            <select
            ...
            >
              <option value="">
                {t("agent.settings.timezone.default", "↩ Company default")}
              </option>
              {[
                "UTC",
                "Asia/Shanghai",
        ...
        <div className="card" style={{ marginBottom: "12px" }}>
          ...
          >
            {t(
              "agent.settings.heartbeat.description",
              "Periodic awareness check — agent proactively monitors the plaza and work environment.",
            )}
          </p>
          <div
          ...
          >
            ...
            >
              <div>
                <div style={{ fontWeight: 500, fontSize: "13px" }}>
                  {t("agent.settings.heartbeat.enabled", "Enable Heartbeat")}
                </div>
                <div
                  style={{ fontSize: "11px", color: "var(--text-tertiary)" }}
                >
                  {t(
                    "agent.settings.heartbeat.enabledDesc",
                    "Agent will periodically check plaza and work status",
                  )}
                </div>
              </div>
              <label
            ...
            >
              <div>
                <div style={{ fontWeight: 500, fontSize: "13px" }}>
                  {t("agent.settings.heartbeat.interval", "Check Interval")}
                </div>
                <div
                  style={{ fontSize: "11px", color: "var(--text-tertiary)" }}
                >
                  {t(
                    "agent.settings.heartbeat.intervalDesc",
            ...
            >
              <div>
                <div style={{ fontWeight: 500, fontSize: "13px" }}>
                  {t("agent.settings.heartbeat.activeHours", "Active Hours")}
                </div>
                <div
                  style={{ fontSize: "11px", color: "var(--text-tertiary)" }}
                >
                  {t(
                    "agent.settings.heartbeat.activeHoursDesc",
                    "Only trigger heartbeat during these hours (HH:MM-HH:MM)",
                  )}
                </div>
              </div>
              <input
            ...
            {agent?.last_heartbeat_at && (
              ...
              >
                {t("agent.settings.heartbeat.lastRun", "Last heartbeat")}:{" "}
        ...
        {canManage && (
          <div className="card" style={{ borderColor: "var(--error)" }}>
            <h4 style={{ color: "var(--error)", marginBottom: "12px" }}>
              {t("agent.settings.danger.title")}
            </h4>
            <p
              style={{
                fontSize: "13px",
            ...
            {!showDeleteConfirm ? (
              ...
              >
                ...
                <button className="btn btn-danger" onClick={onDeleteAgent}>
