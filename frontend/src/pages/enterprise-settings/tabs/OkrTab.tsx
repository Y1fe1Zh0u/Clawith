...
export default function OkrTab({ tenantId, t }: { tenantId: string; t: any }) {
  ...
  const [okrSaveState, setOkrSaveState] = useState<
    "idle" | "saving" | "saved" | "error"
  >("idle");
  const [okrSaveError, setOkrSaveError] = useState("");
  const [dailyTestState, setDailyTestState] = useState<
  ...
  if (isLoading)
    return (
      <div style={{ padding: "20px" }}>{t("common.loading", "Loading...")}</div>
    );
  const s = settings || {
    enabled: false,
  ...
  const periodFrequencyLocked =
    !!s.period_frequency_locked || !!s.first_enabled_at;
  ...
  const okrAgentId: string | null =
    settings?.okr_agent_id ?? membersData?.okr_agent_id ?? null;
    ...
    <div style={{ maxWidth: "800px" }}>
      <div className="card" style={{ marginBottom: "24px" }}>
        ...
        <div
          style={{
            padding: "20px",
            borderBottom: s.enabled ? "1px solid var(--border-subtle)" : "none",
          }}
        >
          <div
            style={{
          ...
          >
            ...
            >
              <input
                ...
                checked={s.enabled}
                onChange={(e) =>
                  saveOkrSettings({ ...s, enabled: e.target.checked })
                }
          ...
          {!s.enabled && !periodFrequencyLocked && (
            <div style={{ marginTop: "20px" }}>
              ...
              >
                {zh
                  ? "首次启用前选择 OKR 周期"
                  : "Choose OKR cadence before first enablement"}
              </div>
              <div
                style={{
                  marginBottom: "10px",
        ...
        {okrSaveState !== "idle" && (
          ...
          >
            {okrSaveState === "saving" &&
              (zh ? "正在保存 OKR 设置..." : "Saving OKR settings...")}
            {okrSaveState === "saved" &&
              (zh ? "OKR 设置已保存" : "OKR settings saved")}
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    saveOkrSettings({
                      ...s,
                      daily_report_enabled: e.target.checked,
                    })
                  }
                />
                <div>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        saveOkrSettings({
                          ...s,
                          daily_report_skip_non_workdays: e.target.checked,
                        })
                      }
                    />
