...
export default function LlmTab({ selectedTenantId }: LlmTabProps) {
  ...
  const openCreateForm = () => {
    ...
    setModelForm({
      ...
      label: "",
      supports_vision: false,
      max_output_tokens: defaultSpec
        ? String(defaultSpec.default_max_tokens)
        : "4096",
      request_timeout: "",
      temperature: "",
    });
      ...
      if (result.connection_success && result.tool_calling_supported === true) {
        if (btn) {
          btn.textContent = result.capability_recorded
            ? t(
                "enterprise.llm.testSuccessWithTools",
                "连接与原生工具调用均可用 · {{latency}}ms",
                { latency: result.latency_ms },
              )
            : t(
                "enterprise.llm.draftTestSuccess",
                "测试通过；保存模型后需再次测试",
              );
        ...
        await dialog.alert(
          capabilityUnknown
            ? t(
                "enterprise.llm.toolTestUnknown",
                "连接成功，但原生工具调用暂未确认",
              )
            : t(
                "enterprise.llm.toolTestUnsupported",
                "连接成功，但原生工具调用不可用",
              ),
          {
            type: "error",
            title: t(
              "enterprise.llm.agentCompatibilityTest",
              "Agent 兼容性测试",
            ),
            details: String(
              result.tool_calling_error ||
                result.error ||
                "Model did not return a valid native capability probe.",
            ),
          },
        );
        ...
        await dialog.alert(
          t("enterprise.llm.testFailedShort", "连通性测试失败"),
          {
            type: "error",
      ...
      {canManageRuntimeModels && runtimeModelSettings && (
        <div className="card" style={{ marginBottom: "16px" }}>
          <div style={{ marginBottom: "14px" }}>
            ...
            >
              {t(
                "enterprise.llm.runtimeModelsHint",
                "可使用当前公司或平台已保存且启用的模型；工具测试结果仅作诊断，不影响选择。保存后立即生效。",
              )}
            </div>
          </div>
          {runtimeModelSettings.candidates.length === 0 ? (
            <div style={{ color: "var(--warning)", fontSize: "13px" }}>
              {t(
                "enterprise.llm.noRuntimeModelCandidates",
                "暂无可用模型，请先为当前公司或平台创建模型并完成 Agent 兼容性测试。",
              )}
            </div>
          ) : (
            <div
              style={{
                display: "grid",
                gridTemplateColumns:
                  "repeat(auto-fit, minmax(260px, 1fr)) auto",
                gap: "12px",
                alignItems: "end",
              }}
            >
              <label className="form-group" style={{ margin: 0 }}>
                ...
                >
                  <option value="" disabled>
                    {t("enterprise.llm.selectRuntimeModel", "请选择模型")}
                  </option>
              ...
              <label className="form-group" style={{ margin: 0 }}>
                <span className="form-label">
                  {t("enterprise.llm.compactModel", "群聊上下文模型")}
                </span>
                <select
                  className="form-input"
        ...
        <div className="card" style={{ marginBottom: "16px" }}>
          ...
          >
            ...
            <div className="form-group">
              ...
              <input
                className="form-input"
                placeholder={t(
                  "enterprise.llm.modelPlaceholder",
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setModelForm({ ...modelForm, base_url: e.target.value })
                }
              />
            </div>
            ...
            <div className="form-group" style={{ gridColumn: "span 2" }}>
              ...
              >
                ...
                >
                  {t("enterprise.llm.supportsVisionDesc")}
                </span>
              </label>
            </div>
            <div className="form-group">
              <label className="form-label">
                {t("enterprise.llm.maxOutputTokens", "Max Output Tokens")}
              </label>
              <input
                className="form-input"
                type="number"
                placeholder={t(
                  "enterprise.llm.maxOutputTokensPlaceholder",
                  "e.g. 4096",
                )}
              ...
              >
                {t(
                  "enterprise.llm.maxOutputTokensDesc",
            ...
            <div className="form-group">
              <label className="form-label">
                {t("enterprise.llm.requestTimeout", "Request Timeout (s)")}
              </label>
              <input
                className="form-input"
                type="number"
                min="1"
                placeholder={t(
                  "enterprise.llm.requestTimeoutPlaceholder",
                  "e.g. 120 (Leave empty for default)",
                )}
                value={modelForm.request_timeout}
                onChange={(e) =>
                  setModelForm({
                    ...modelForm,
              ...
              >
                {t(
                  "enterprise.llm.requestTimeoutDesc",
            ...
            <div className="form-group">
              <label className="form-label">
                {t("enterprise.llm.temperature", "Temperature")}
              </label>
              <input
                className="form-input"
                type="number"
                ...
                max="2"
                placeholder={t(
                  "enterprise.llm.temperaturePlaceholder",
                  "e.g. 0.7 or 1.0 (Leave empty for default)",
                )}
                value={modelForm.temperature}
                onChange={(e) =>
                  setModelForm({ ...modelForm, temperature: e.target.value })
                }
              />
              <div
                style={{
              ...
              >
                {t(
                  "enterprise.llm.temperatureDesc",
                  "Leave empty to use the provider default. o1/o3 reasoning models usually require 1.0",
                )}
              </div>
            </div>
          </div>
          ...
          >
            ...
            <button
              ...
              onClick={() => runConnectivityTest(null, true)}
            >
              ...
              onClick={() => {
                addModel.mutate({
                  ...
                  max_output_tokens: modelForm.max_output_tokens
                    ? Number(modelForm.max_output_tokens)
                    : null,
                  request_timeout: modelForm.request_timeout
                    ? Number(modelForm.request_timeout)
                    : null,
                  temperature:
                    modelForm.temperature !== ""
                      ? Number(modelForm.temperature)
                      : null,
                });
              }}
              disabled={!modelForm.model || !modelForm.api_key}
              ...
              >
                  ...
                  <div className="form-group">
                    ...
                    >
                      ...
                      {!providerOptions.some(
                        (p) => p.provider === modelForm.provider,
                      ) && (
                        <option value={modelForm.provider}>
                          {modelForm.provider}
                        </option>
                      )}
                    </select>
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => runConnectivityTest(editingModelId, false)}
                  >
                    {t("enterprise.llm.test")}
                  </button>
                  <button
              ...
              >
                ...
                >
                  <button
                    ...
                    title={
                      m.enabled
                        ? t("enterprise.llm.clickToDisable", "Click to disable")
                        : t("enterprise.llm.clickToEnable", "Click to enable")
                    }
                    style={{
                      position: "relative",
                      width: "36px",
                  ...
                  {m.supports_tool_calling === true ? (
                    <span
                      ...
                      title={
                        m.tool_calling_capability_source === "probe"
                          ? t(
                              "enterprise.llm.toolsVerifiedTitle",
                              "This exact model configuration passed the native tool-calling probe.",
                            )
                          : t(
                              "enterprise.llm.toolsRegistryTitle",
                              "Preserved from the built-in cloud provider registry; rerun the test to verify this exact configuration.",
                            )
                      }
                      style={{
                    ...
                    <span
                      className="badge"
                      title={
                        m.tool_calling_error ||
                        t(
                          "enterprise.llm.toolsUnavailableTitle",
                          "The tool probe failed, but the saved enabled model remains available.",
                        )
                      }
                      style={{
                    ...
                    >
                      {t(
                        "enterprise.llm.toolsUnavailable",
                        "Tools unavailable",
                      )}
                    </span>
                  ) : (
                    <span
                      ...
                      title={
                        m.tool_calling_error ||
                        t(
                          "enterprise.llm.toolsUnverifiedTitle",
                          "Tool support has not been tested; the saved enabled model remains available.",
                        )
                      }
                      style={{
                  ...
                  {tenantForDefault?.default_model_id === m.id ? (
                    ...
                    >
                      {t("enterprise.llm.defaultBadge", "默认")}
                    </span>
                  ) : m.enabled ? (
                    <button
                      ...
                      title={t(
                        "enterprise.llm.setAsDefaultTitle",
                  ...
                  <button
                    className="btn btn-ghost"
                    onClick={() => void confirmDeleteModel(m)}
