...
export default function SkillsTab() {
  ...
  const adapter: FileBrowserApi = useMemo(
    () => ({
      ...
      write: (path: string, content: string) =>
        skillApi.browse.write(path, content),
  ...
  const handleInstall = async (slug: string) => {
    ...
    try {
      ...
      showToast(
        `Installed "${result.name}" — ${tierLabel}, ${result.file_count} files`,
      );
      setRefreshKey((k) => k + 1);
    ...
    const styles: Record<number, { bg: string; color: string; label: string }> =
      {
        1: {
          ...
          label: "Tier 1 · Pure Prompt",
        },
        2: {
          bg: "rgba(255,159,10,0.12)",
          color: "var(--warning, #ff9f0a)",
          label: "Tier 2 · CLI/API",
        },
        3: {
          bg: "rgba(255,59,48,0.12)",
          color: "var(--error, #ff3b30)",
          label: "Tier 3 · OpenClaw Native",
        },
      };
      ...
      {showSettings && (
        ...
        >
            ...
            >
              ...
              >
                <div style={{ marginBottom: "6px", fontWeight: 500 }}>
                  {t("enterprise.tools.howToGenerateGithubToken")}
                </div>
                {t("enterprise.tools.githubTokenStep1")}
                <br />
                {t("enterprise.tools.githubTokenStep2")}
                <br />
                {t("enterprise.tools.githubTokenStep3")}
                <br />
                {t("enterprise.tools.githubTokenStep4")}
                <br />
                {t("enterprise.tools.githubTokenStep5")}
                <br />
                <div
                  style={{
                    marginTop: "6px",
          ...
          {tokenStatus?.configured && (
            ...
            >
              {t("enterprise.tools.currentToken")}{" "}
              <code
                style={{
                  padding: "2px 6px",
                  borderRadius: "4px",
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            >
              {savingToken
                ? t("enterprise.tools.saving")
                : t("enterprise.tools.save")}
            </button>
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <input
                type="text"
                name="prevent_autofill_ch_user"
                style={{ display: "none" }}
                tabIndex={-1}
              />
              <input
                type="password"
                name="prevent_autofill_ch_pass"
                style={{ display: "none" }}
                tabIndex={-1}
              />
              <input
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                >
                  {searching
                    ? t("enterprise.tools.searching")
                    : t("enterprise.tools.search")}
                </button>
              </div>
            </div>
            {/* Results */}
            <div style={{ flex: 1, overflowY: "auto", padding: "12px 24px" }}>
              {searchResults.length === 0 && !searching && (
                ...
                >
                  {hasSearched
                    ? t("enterprise.tools.noResultsFound")
              ...
              {searchResults.map((r: any) => (
                ...
                >
                  ...
                  >
                    {installing === r.slug
                      ? t("enterprise.tools.installing")
                      : t("enterprise.tools.install")}
                  </button>
                </div>
              ))}
            </div>
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button
                  ...
                  disabled={urlPreviewing || !urlInput.trim()}
                  style={{ fontSize: "12px" }}
                >
                  {urlPreviewing
                    ? t("enterprise.tools.loading")
                    : t("enterprise.tools.preview")}
                </button>
              </div>
            </div>
            ...
            {urlPreview && (
              <div style={{ padding: "16px 24px" }}>
                ...
                >
                  {urlPreview.files?.length} files,{" "}
                  {(urlPreview.total_size / 1024).toFixed(1)} KB
                </div>
                <div
                  style={{
                    display: "flex",
...
function CompanyLogoCropModal({
  ...
  return (
    <div className="tenant-logo-crop-backdrop" onClick={onCancel}>
      <div
        className="tenant-logo-crop-modal"
        onClick={(e) => e.stopPropagation()}
      >
...
function CompanyLogoEditor() {
  ...
  const handleLogoFile = (file: File | undefined) => {
    ...
    if (file.size > 1024 * 1024) {
      setLogoError(
        t("enterprise.logo.tooLarge", "Logo image must be 1 MB or smaller."),
      );
      return;
    }
    if (!file.type.startsWith("image/")) {
      setLogoError(
        t("enterprise.logo.invalidType", "Please choose an image file."),
      );
      return;
    }
  ...
  const uploadCroppedLogo = async (blob: Blob) => {
    ...
    if (blob.size > 1024 * 1024) {
      setLogoError(
        t(
          "enterprise.logo.croppedTooLarge",
          "Cropped logo is still larger than 1 MB.",
        ),
      );
      return;
    }
  ...
  return (
    <div className="card" style={{ padding: "16px", marginBottom: "24px" }}>
      ...
      >
        {t(
          "enterprise.logo.description",
          "Used in the sidebar workspace switcher and company selection menus.",
        )}
      </div>
      <div className="company-identity-logo-row">
        ...
        <div>
          ...
          >
            {logoSaving
              ? t("common.loading")
              : t("enterprise.logo.upload", "Upload logo")}
          </button>
          {logoUrl && (
            <button
              className="btn btn-ghost"
          ...
          >
            {t(
              "enterprise.logo.hint",
              "PNG, JPG, or WebP. Max 1 MB. You will crop it to a square before saving.",
            )}
          </div>
          {logoError && (
            <div
    ...
    <div className="card" style={{ padding: "16px", marginBottom: "24px" }}>
      ...
      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
        <input
          ...
          placeholder={t(
            "enterprise.companyName.placeholder",
            "Enter company name",
          )}
        ...
        {saved && (
          <IconCheck size={15} stroke={2} style={{ color: "var(--success)" }} />
        )}
      </div>
    </div>
  );
...
function CompanyTimezoneEditor() {
  ...
  const tenantId =
    user?.tenant_id || localStorage.getItem("current_tenant_id") || "";
  ...
  const companyRegions = useMemo(
    () => buildCompanyRegions(zh ? "zh-Hans" : "en"),
    [zh],
  );
  ...
  const handleSave = async (regionCode: string) => {
    if (!tenantId) return;
    const region =
      companyRegions.find((r) => r.code === regionCode) || companyRegions[0];
  ...
  return (
    <div className="card" style={{ padding: "16px", marginBottom: "24px" }}>
      <div style={{ fontSize: "13px", fontWeight: 500, marginBottom: "4px" }}>
        {t(
          "enterprise.timezone.countryRegionTitle",
      ...
      >
        {t("enterprise.timezone.countryRegionDescription", {
          timezone,
          defaultValue:
      ...
      >
        <div
          ref={regionPickerRef}
          style={{ position: "relative", width: "min(420px, 100%)" }}
        >
          <input
            className="form-input"
            value={regionInput}
            ...
            onKeyDown={(e) => {
              if (e.key === "ArrowDown") {
                ...
                setHighlightedRegion((i) =>
                  Math.min(i + 1, Math.max(filteredRegions.length - 1, 0)),
                );
            ...
            placeholder={t(
              "enterprise.timezone.searchPlaceholder",
              "Search country, code, or timezone",
            )}
            style={{
              width: "100%",
              fontSize: "13px",
          ...
          <button
            ...
            aria-label={
              regionOpen
                ? t(
                    "enterprise.timezone.collapseRegionList",
                    "Collapse region list",
                  )
                : t(
                    "enterprise.timezone.expandRegionList",
                    "Expand region list",
                  )
            }
            style={{
        ...
        {(saved || error || !tenantId) && (
          ...
          >
            ...
            {!tenantId && (
              <div style={{ fontSize: "11px", color: "var(--error)" }}>
                {t(
                  "enterprise.timezone.noTenant",
                  "No company selected. Please refresh the page or contact support.",
                )}
              </div>
            )}
          </div>
...
function A2AAsyncToggle() {
  ...
  return (
    <div className="card" style={{ padding: "16px", marginBottom: "24px" }}>
      ...
      >
        {t(
          "enterprise.a2aAsync.description",
  ...
  return (
    <div style={{ marginTop: "24px", marginBottom: "24px" }}>
      <h3 style={{ marginBottom: "4px" }}>
        {t("enterprise.broadcast.title", "Broadcast Notification")}
      </h3>
      <p
        style={{
          fontSize: "12px",
      ...
      >
        {t(
          "enterprise.broadcast.description",
          "Send a notification to all users and agents in this company.",
        )}
      </p>
      <div className="card" style={{ padding: "16px" }}>
        <input
          className="form-input"
          placeholder={t(
            "enterprise.broadcast.titlePlaceholder",
            "Notification title",
          )}
          value={title}
        ...
        <textarea
          className="form-input"
          placeholder={t(
            "enterprise.broadcast.bodyPlaceholder",
            "Optional details...",
          )}
          value={body}
        ...
        >
          ...
          <span>
            {t(
              "enterprise.broadcast.sendEmail",
              "Also send email to users with a configured address",
            )}
          </span>
        </label>
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          >
            {sending
              ? t("common.loading")
              : t("enterprise.broadcast.send", "Send Broadcast")}
          </button>
          {result && (
            <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
              {t(
