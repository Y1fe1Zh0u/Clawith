...
function objectiveProgress(obj: Objective): number {
  ...
  const avg =
    obj.key_results.reduce((s, kr) => s + progressPercent(kr), 0) /
...
function StatusBadge({
  ...
  const label = isChinese
    ? (STATUS_LABELS[status]?.zh ?? status)
...
function KRCard({
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: "13px", color: "var(--text-primary)", flex: 1 }}
        >
          {kr.title}
        </span>
        <div
            ...
            <button
              onClick={async () => {
                const ok = await dialog.confirm(
                  isChinese
                    ? "确定要删除这个 Key Result 吗？此操作不可恢复。"
                    : "Are you sure you want to delete this Key Result?",
                  {
                    title: isChinese ? "删除 Key Result" : "Delete Key Result",
                    danger: true,
      ...
      {editing && (
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              >
                ...
                <option value="on_track">
                  {isChinese ? "按计划" : "On Track"}
                </option>
                <option value="at_risk">
                  {isChinese ? "有风险" : "At Risk"}
                </option>
          ...
          <input
            ...
            placeholder={
              isChinese ? "更新说明（可选）" : "Update note (optional)"
            }
            style={{
              padding: "6px 10px",
              background: "var(--bg-primary)",
...
function AddKRForm({
  ...
  async function handleSubmit() {
    if (!title.trim()) {
      setError(
        isChinese ? "请输入 Key Result 描述" : "Please enter a description",
      );
      return;
    }
    setSaving(true);
  ...
  return (
    ...
    >
      <input
        ...
        placeholder={
          isChinese
            ? "Key Result 描述，例如：用户满意度达到 4.5 分"
            : "e.g. Increase NPS to 50"
        }
        autoFocus
        style={{
          padding: "6px 10px",
      ...
      <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
        ...
        <input
          ...
          placeholder={
            isChinese ? "单位（可选，如 %、万元）" : "Unit (e.g. %, pts)"
          }
          style={{
            flex: 1,
            padding: "4px 8px",
...
function ObjectiveCard({
  ...
  const overallStatus =
    obj.status === "completed" ? "completed" : deriveStatus(pct);
          ...
          {canEdit && onDelete && (
            <button
              onClick={async (e) => {
                ...
                const ok = await dialog.confirm(
                  isChinese
                    ? "确定要删除这个目标及其所有相关的 Key Results 吗？（此操作实际上是将目标归档）"
                    : "Are you sure you want to delete this Objective and all its Key Results? (This will archive the objective)",
                  {
                    title: isChinese
                      ? "删除 / 归档目标"
                      : "Delete / Archive Objective",
                    danger: true,
                    confirmLabel: isChinese
                      ? "删除 / 归档"
                      : "Delete / Archive",
                  },
                );
        ...
        >
          {obj.key_results.map((kr) => (
            <KRCard
              ...
              onDelete={async (krId) => {
                await fetchJson(`/okr/key-results/${krId}`, {
                  method: "DELETE",
                });
                onInvalidate();
              }}
...
function CreateObjectiveForm({
  ...
  const [ownerType, setOwnerType] = useState<"company" | "user">(
  ...
  async function handleSubmit() {
    if (!title.trim()) {
      setError(isChinese ? "请输入目标标题" : "Please enter a title");
      return;
    }
    setSaving(true);
    setError("");
...
export default function OKR() {
      ...
      >
        ...
        {isAdmin && (
          ...
          >
            {isChinese
              ? "前往公司设置开启 OKR"
              : "Enable OKR in Company Settings"}
          </button>
        )}
        {!isAdmin && (
          <div style={{ fontSize: "12px", color: "var(--text-quaternary)" }}>
            {isChinese
              ? "请联系管理员开启 OKR 功能"
              : "Please ask an admin to enable OKR"}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div
      data-okr-role-mode={okrRoleMode}
              ...
              >
                ...
                >
                  {t(
                    ...
                    isChinese ? "公司目标" : "Company Objectives",
                  )}
                </span>
                <div
                  style={{
              ...
              >
                {companyObjs.map((obj) => (
                  <ObjectiveCard
                    ...
                    onDelete={async (id) => {
                      await fetchJson(`/okr/objectives/${id}`, {
                        method: "DELETE",
                      });
                      invalidateObjectives();
                    }}
          ...
          {!objLoading && Object.keys(memberGroups).length > 0 && (
            <section>
              ...
              >
                ...
                >
                  {t(
                    "okr.memberObjectives",
                    isChinese ? "成员目标" : "Member Objectives",
                  )}
                </span>
                <div
                  style={{
      ...
      setTimeout(
        () =>
          queryClient.invalidateQueries({
            queryKey: ["okr-members-without-okr"],
          }),
        5000,
      );
      setTimeout(
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {channel_warnings.map((w, i) => (
              <div
                key={i}
                style={{
                  marginBottom: i < channel_warnings.length - 1 ? "4px" : 0,
                }}
              >
                <span style={{ fontWeight: 600 }}>
                  {w.affected_members.join("、")}
                </span>{" "}
                {isChinese
              ...
              {unreachableNames.has(member.display_name) && (
                <span
                  title={
                    isChinese
                      ? "渠道未配置，无法推送"
                      : "Channel not configured, cannot deliver"
                  }
                  style={{
                    cursor: "help",
                    flexShrink: 0,
...
function ReportsTab({ isChinese }: { isChinese: boolean }) {
  ...
  const [expandedCompanyReportId, setExpandedCompanyReportId] = useState<
    string | null
  >(null);
  const [selectedMemberReportId, setSelectedMemberReportId] = useState<
    string | null
  >(null);
  ...
  const { data: companyReports = [], isLoading: companyLoading } = useQuery<
    CompanyReport[]
  >({
  ...
  const { data: memberReports = [], isLoading: memberLoading } = useQuery<
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: "8px 14px",
                background:
                  view === item.key ? "var(--bg-primary)" : "transparent",
                color:
                  view === item.key
                    ? "var(--text-primary)"
                    : "var(--text-secondary)",
                ...
                cursor: "pointer",
                boxShadow:
                  view === item.key ? "0 1px 4px rgba(0,0,0,0.06)" : "none",
              }}
            >
            ...
            <div style={{ display: "inline-flex", gap: "6px" }}>
              {[
                ...
                <button
                  key={item.key}
                  onClick={() =>
                    setReportType(item.key as "daily" | "weekly" | "monthly")
                  }
                  style={{
          ...
          >
            ...
            >
              <input
                ...
                style={{ width: "100%" }}
              />
              {filteredMemberReports.length ? (
                filteredMemberReports.map((item) => (
                  <div
                    key={item.id}
                    ...
                    style={{
                      ...
                      background:
                        selectedMemberReport?.id === item.id
              ...
              {selectedMemberReport ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {selectedMemberReport.updated_at
                        ? new Date(
                            selectedMemberReport.updated_at,
                          ).toLocaleString()
                        : ""}
                  ...
                  <div
                    style={{
                      fontSize: "13px",
                      lineHeight: "1.7",
                      color: selectedMemberReport.content
                        ? "var(--text-secondary)"
                        : "var(--text-tertiary)",
                      whiteSpace: "pre-wrap",
                      wordBreak: "break-word",
                ...
                >
                  {isChinese
                    ? "当天暂无成员日报。"
                    : "No member daily reports for this day."}
                </div>
              )}
            </div>
          </div>
