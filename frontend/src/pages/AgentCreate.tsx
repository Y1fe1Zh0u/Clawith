...
export default function AgentCreate() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (agent) => {
      ...
      if (channelValues.feishu_app_id && channelValues.feishu_app_secret) {
        try {
          await channelApi.create(agent.id, {
            ...
            extra_config: {
              connection_mode:
                channelValues.feishu_connection_mode || "websocket",
            },
          });
        } catch (err) {
      ...
      // Discord
      if (
        channelValues.discord_bot_token &&
        channelValues.discord_application_id
      ) {
        try {
          await channelApi.create(agent.id, {
      ...
      if (channelValues.wecom_bot_id && channelValues.wecom_bot_secret) {
        try {
          ...
          await channelApi.create(agent.id, {
            channel_type: "wecom",
            app_id:
              connMode === "websocket" ? channelValues.wecom_bot_id : undefined,
            app_secret:
              connMode === "websocket"
                ? channelValues.wecom_bot_secret
                : undefined,
            extra_config: {
  ...
  const validateStep0 = (): boolean => {
    ...
    if (
      form.max_tokens_per_day &&
      (isNaN(Number(form.max_tokens_per_day)) ||
        Number(form.max_tokens_per_day) <= 0)
    ) {
      errors.max_tokens_per_day = t(
        "wizard.errors.tokenLimitInvalid",
        "请输入有效的正整数",
      );
    }
    if (
      form.max_tokens_per_month &&
      (isNaN(Number(form.max_tokens_per_month)) ||
        Number(form.max_tokens_per_month) <= 0)
    ) {
      errors.max_tokens_per_month = t(
        "wizard.errors.tokenLimitInvalid",
        "请输入有效的正整数",
      );
    }
    ...
    if (
      ...
      errors.primary_model_id = t(
        "wizard.errors.modelRequired",
        "请选择一个主模型",
      );
    }
  ...
  const handleFinish = () => {
    ...
    createMutation.mutate({
      ...
      primary_model_id:
        agentType === "native" ? form.primary_model_id || undefined : undefined,
      fallback_model_id:
        agentType === "native"
          ? form.fallback_model_id || undefined
          : undefined,
      permission_scope_type: form.permission_scope_type,
      max_tokens_per_day: form.max_tokens_per_day
        ? Number(form.max_tokens_per_day)
        : undefined,
      max_tokens_per_month: form.max_tokens_per_month
  ...
  if (createdApiKey && createMutation.data) {
    ...
    const setupInstruction = buildOpenClawInstruction(
      createdApiKey,
      ...
      <div>
        <div className="page-header">
          <h1 className="page-title">
            {t("openclaw.created", "OpenClaw Agent Created")}
          </h1>
        </div>
        <div className="card" style={{ maxWidth: "640px" }}>
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            >
              {t(
                "openclaw.createdDesc2",
                "Your OpenClaw agent has been registered. Copy the instruction below and send it to your OpenClaw agent to complete the setup.",
              )}
            </p>
          </div>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            >
              {t(
                "openclaw.setupInstructionDesc",
                "Copy and send this to your OpenClaw agent. It will configure itself automatically.",
              )}
            </p>
          ...
          <details style={{ marginBottom: "24px" }}>
            ...
            <div style={{ marginTop: "8px" }}>
              ...
              >
                {t(
                  "openclaw.keyNote",
                  "This key is already embedded in the instruction above. Save it separately if needed for manual configuration.",
                )}
              </p>
            </div>
          </details>
      ...
      <div>
        ...
        <div className="card" style={{ maxWidth: "640px" }}>
          ...
          >
            {t(
              "openclaw.basicDesc",
              "Give your OpenClaw agent a name and description. The LLM model, personality, and skills are configured on your OpenClaw instance.",
            )}
          </p>
          ...
          <div className="form-group">
            ...
            <input
              className={`form-input${fieldErrors.role_description ? " input-error" : ""}`}
              ...
              placeholder={t(
                "openclaw.rolePlaceholder",
                "e.g. Personal assistant running on my Mac",
              )}
            />
              ...
              {(
                ...
                <label
                  ...
                  style={{
                    ...
                    padding: "12px",
                    background:
                      form.permission_scope_type === scope.value
                        ? "var(--accent-subtle)"
                        : "var(--bg-elevated)",
                ...
                >
                  <input
                    type="radio"
                    name="scope"
                    checked={form.permission_scope_type === scope.value}
                    onChange={() =>
                      setForm({ ...form, permission_scope_type: scope.value })
                    }
                  />
                  <div>
          ...
          >
            ...
            >
              {createMutation.isPending
                ? t("common.loading")
                : t("openclaw.createBtn", "Link Agent")}
            </button>
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: "640px" }}>
        ...
        {step === 0 && (
          <div>
                    ...
                    .map((m: LlmModel) => (
                      ...
                      >
                        <input
                          type="radio"
                          name="model"
                          checked={form.primary_model_id === m.id}
                          onChange={() => {
                            setForm({ ...form, primary_model_id: m.id });
            ...
            >
              <div className="form-group">
                <label className="form-label">
                  {t("wizard.step1.dailyTokenLimit")}
                </label>
                <input
                  className={`form-input${fieldErrors.max_tokens_per_day ? " input-error" : ""}`}
                  type="number"
                  value={form.max_tokens_per_day}
                  onChange={(e) => {
                    setForm({ ...form, max_tokens_per_day: e.target.value });
                    clearFieldError("max_tokens_per_day");
                  }}
                  placeholder={t("wizard.step1.unlimited")}
                />
                {fieldErrors.max_tokens_per_day && (
                  <div
                    style={{
                      color: "var(--error)",
                      fontSize: "12px",
              ...
              <div className="form-group">
                <label className="form-label">
                  {t("wizard.step1.monthlyTokenLimit")}
                </label>
                <input
                  className={`form-input${fieldErrors.max_tokens_per_month ? " input-error" : ""}`}
                  type="number"
                  value={form.max_tokens_per_month}
                  onChange={(e) => {
                    setForm({ ...form, max_tokens_per_month: e.target.value });
                    clearFieldError("max_tokens_per_month");
                  }}
                  placeholder={t("wizard.step1.unlimited")}
                />
                {fieldErrors.max_tokens_per_month && (
                  <div
                    style={{
                      color: "var(--error)",
                      fontSize: "12px",
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) {
                          setForm({
                            ...form,
                            skill_ids: [...form.skill_ids, skill.id],
                          });
                        } else {
                          setForm({
                            ...form,
                            skill_ids: form.skill_ids.filter(
                    ...
                    >
                      {/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}]/u.test(
                        ...
                        skill.icon || <IconTools size={18} stroke={1.8} />
                      )}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div
                  ...
                  {(
                    ...
                    <label
                      ...
                      style={{
                        ...
                        padding: "12px",
                        background:
                          form.permission_access_level === lvl.value
                            ? "var(--accent-subtle)"
                            : "var(--bg-elevated)",
                    ...
                    >
                      <input
                        type="radio"
                        name="access_level"
                        checked={form.permission_access_level === lvl.value}
                        onChange={() =>
                          setForm({
                            ...form,
                            permission_access_level: lvl.value,
                          })
                        }
                        style={{ marginTop: "2px" }}
                      />
        ...
        {step === 4 && (
          <div>
            ...
            >
              {t("wizard.step5.title", "Channel Configuration")}
            </h3>
            <p
              style={{
                fontSize: "13px",
            ...
            >
              {t(
                ...
                "Connect messaging platforms to enable your agent to communicate through different channels.",
              )}
            </p>
      ...
      {selectedModel && (
        ...
        >
          ...
          {t("wizard.summary.model")}: {selectedModel.label}
          {form.max_tokens_per_day &&
            ` · ${t("wizard.summary.dailyLimit")}: ${Number(form.max_tokens_per_day).toLocaleString()}`}
        </div>
      )}
