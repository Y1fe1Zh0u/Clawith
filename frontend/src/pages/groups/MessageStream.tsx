...
function renderContentWithMentions(
  content: string,
  mentions: GroupMessage["mentions"],
) {
  ...
  return parts.map((part, index) =>
    index % 2 === 1 ? (
      <span key={index} className="group-mention-chip">
        @{part}
      </span>
    ) : (
...
export default function MessageStream({
  ...
  const reportLatestMessageSeen = useCallback(() => {
    ...
    if (
      bottomRect.top < streamRect.top ||
      bottomRect.bottom > streamRect.bottom + 1
    )
      return;
    onLatestMessageSeen(latestMessageId);
  }, [latestMessageId, onLatestMessageSeen]);

  const memberByParticipant = new Map(
    ...
    >
      {hasMore && (
        <div className="group-stream-more">
          <button
            ...
            onClick={() => void historyLoadGesture.requestOlder()}
      ...
      {messages.map((message) => {
        ...
        const isMine =
          ...
          message.participant_id === myParticipantId;
        const name =
          message.sender_name ??
          member?.display_name ??
        ...
        return (
          <div
            key={message.id}
            className={`group-message ${isMine ? "mine" : ""}`}
          >
      ...
      {runningAgents.map((agent) => (
        ...
        >
          ...
          <div className="group-message-body">
            <div className="group-message-meta">
              <span className="group-message-sender">{agent.name}</span>
              <span className="group-badge-agent">
                {t("groups.agentBadge", "智能体")}
              </span>
            </div>
