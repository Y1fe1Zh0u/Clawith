...
export const GROUP_WORKSPACE_BINARY_EXTENSIONS =
  WORKSPACE_BINARY_UPLOAD_EXTENSIONS;
...
export type GroupWorkspaceUploadErrorCode =
  "invalid_name" | "unsupported_type" | "invalid_utf8";
...
export function isGroupWorkspaceTextUpload(name: string): boolean {
  ...
  return (
    GROUP_WORKSPACE_TEXT_EXTENSIONS.some((extension) =>
...
function isSupportedUploadName(name: string): boolean {
  const lower = name.toLowerCase();
  return (
    isGroupWorkspaceTextUpload(name) ||
    GROUP_WORKSPACE_BINARY_EXTENSIONS.some((extension) =>
      lower.endsWith(extension),
    )
  );
}

export function groupWorkspaceUploadPath(
  directory: string,
  fileName: string,
): string {
  if (
