...
export default function GroupSidePanel({
  ...
  const agents = members.filter(
    (member) => member.participant_type === "agent",
  );

  const renderMember = (member: GroupMember) => (
    <div key={member.id} className="group-member-row">
      <span
        className={`group-avatar sm ${member.participant_type === "agent" ? "agent" : ""}`}
      >
      ...
      <div className="group-member-body">
        <div className="group-member-name">
          {member.display_name}
          {member.role === "manager" && (
            <span className="group-badge-manager">
              {t("groups.manager", "群管理")}
            </span>
          )}
          {member.is_deleted && (
  ...
  const TABS: { key: PanelTab; label: string }[] = [
    {
      key: "members",
      label: `${t("groups.members", "成员")} · ${members.length}`,
    },
  ...
  return (
    <aside className="group-side-panel" style={{ width, flexBasis: width }}>
      ...
      <div className="group-panel-topbar">
        <span className="group-panel-groupname" title={groupName}>
          {groupName}
        </span>
      ...
      <div className="group-panel-body">
        ...
        {tab === "announcement" && (
          <GroupTextFileEditor
            ...
            note={t(
              "groups.announcementNote",
              "群公告会注入被 @ 智能体的上下文，用于约定群目标和协作规则。",
            )}
            placeholder={t(
              "groups.announcementPlaceholder",
              "写下群目标、协作规则和对智能体的要求...",
            )}
            load={() => groupApi.announcement(groupId)}
            save={(content, token) =>
              groupApi.saveAnnouncement(groupId, content, token)
            }
          />
        )}
        ...
        {tab === "memory" && (
          <GroupMemoryTab groupId={groupId} members={members} />
        )}
      </div>
    </aside>
  );
