...
function editFromHint(
  ...
  const insertedLength =
    nextValue.length - (previousValue.length - (oldEnd - start));
  ...
  if (nextValue.slice(start + insertedLength) !== previousValue.slice(oldEnd))
    return null;
...
function inferSingleEdit(previousValue: string, nextValue: string): TextEdit {
  ...
  while (start < sharedLength && previousValue[start] === nextValue[start])
    start += 1;
...
function isLiveBinding(value: string, binding: MentionBinding): boolean {
  ...
  const continuesName =
    after !== undefined && /[\p{L}\p{N}\p{M}_@]/u.test(after);
...
export function reconcileMentionBindings(
  ...
  const edit =
    (hint && editFromHint(previousValue, nextValue, hint)) ||
...
export function replaceMentionBinding(
  ...
  const untouched = bindings.filter(
    (binding) =>
...
export function liveMentionedAgentCount(
  value: string,
  bindings: MentionBinding[],
): number {
