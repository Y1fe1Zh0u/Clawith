...
export default function GroupsPage() {
  ...
  const [deletingSession, setDeletingSession] = useState<GroupSession | null>(
    null,
  );
  const [deletingGroup, setDeletingGroup] = useState(false);
  const [showSettings, setShowSettings] = useState(false);
  const groupActivityRefreshTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  const appliedRealtimeMessageIdsRef = useRef<Set<string>>(new Set());
  const latestRealtimeMessageBySessionRef = useRef<Map<string, GroupMessage>>(
    new Map(),
  );
  ...
  const groupRollups = useMemo(() => {
    ...
    for (const group of groups) {
      ...
      const unread = groupSessions.reduce(
        (sum, session) => sum + session.unread_count,
        0,
      );
      const lastActivity = groupSessions.reduce<string | null>(
        (latest, session) => {
          if (!session.last_message_at) return latest;
          return !latest || session.last_message_at > latest
  ...
  const pendingWorkspaceReconciliations = activeRunStates.flatMap((run) =>
    run.correlation_id
      ? run.pending_tool_reconciliations
          .filter(
            (pending) => pending.can_reconcile && pending.workspace_resolution,
          )
  ...
  const memberCounts = useMemo(
    () => ({
      agents: members.filter((member) => member.participant_type === "agent")
        .length,
      people: members.filter((member) => member.participant_type === "user")
        .length,
    }),
    [members],
  );
  ...
  useEffect(() => {
    ...
    const landing =
      sessions.find((session) => session.is_primary) ?? sessions[0];
    ...
    (activity: GroupActivity) => {
      ...
      if (isUnreadForMe && message && activitySessionId) {
        const previousLatest =
          latestRealtimeMessageBySessionRef.current.get(activitySessionId);
        if (
          !previousLatest ||
          compareCursor(message.cursor, previousLatest.cursor) > 0
        ) {
          latestRealtimeMessageBySessionRef.current.set(
            activitySessionId,
            message,
          );
        }
      ...
      if (
        ...
        if (appliedRealtimeMessageIdsRef.current.size > 1000) {
          const oldestMessageId = appliedRealtimeMessageIdsRef.current
            .values()
            .next().value;
          if (oldestMessageId)
            appliedRealtimeMessageIdsRef.current.delete(oldestMessageId);
        }
  ...
  const markLatestMessageSeen = useCallback(
    (messageId: string) => {
      ...
      void groupApi
        .markSessionRead(targetGroupId, targetSessionId, messageId)
        .then(async (readState) => {
          ...
          const latestRealtimeMessageId =
            latestRealtimeMessageBySessionRef.current.get(targetSessionId)?.id;
          if (
  ...
  const toggleGroups = persistToggle(
  ...
  const sendMessage = async (
    ...
    try {
      const intake = await groupApi.sendMessage(groupId, sessionId, {
        content,
        mentions: mentionParticipantIds.map((participant_id) => ({
          participant_id,
        })),
  ...
  const cancelActiveRuns = async () => {
    if (!groupId || !sessionId || activeRunIds.length === 0 || cancellingRuns)
      return;
    setCancellingRuns(true);
    try {
      await Promise.all(
        activeRunIds.map((runId) =>
          groupApi.cancelRun(groupId, sessionId, runId),
        ),
      );
  ...
  const reconcileWorkspaceCandidate = async (
    ...
    if (!groupId || !sessionId || !run.correlation_id || reconcilingExecutionId)
      return;
  ...
  const deleteSession = async () => {
    ...
    try {
      ...
      if (deletingSession.id === sessionId) {
        ...
        navigate(
          next ? `/groups/${groupId}/${next.id}` : `/groups/${groupId}`,
          { replace: true },
        );
      }
    } catch (error) {
  ...
  return (
    <div className="groups-page">
      <div
        className={`group-column tree ${groupsCollapsed ? "collapsed" : ""}`}
      >
        {groupsCollapsed ? (
          <button
            type="button"
              ...
              {groups.map((group) => {
                ...
                return (
                  <div className="group-node" key={group.id}>
                    <div
                      className={`group-row group ${isActiveGroup ? "active" : ""}`}
                    >
                      <button
                        type="button"
            ...
            <header className="group-main-header">
              <div className="group-main-heading">
                ...
                <div className="group-main-subtitle">
                  {t("groups.agentCount", "{{count}} 个智能体", {
                    count: memberCounts.agents,
                  })}
                  {" · "}
                  {t("groups.memberCount", "{{count}} 位成员", {
                    count: memberCounts.people,
                  })}
                </div>
              </div>
              <button
              ...
              >
                ...
                <div className="chat-tool-reconciliation__actions">
                  <button
                    ...
                    onClick={() =>
                      void reconcileWorkspaceCandidate(
                        run,
                        pending,
                        "not_applied",
                      )
                  ...
                  <button
                    ...
                    disabled={reconcilingExecutionId !== null}
                    onClick={() =>
                      void reconcileWorkspaceCandidate(run, pending, "applied")
                    }
                  >
                    使用 Agent 的结果
                  </button>
      ...
      <ConfirmModal
        ...
        message={t(
          "groups.deleteGroupConfirm",
          "删除后该群的所有会话、消息和文件都将无法恢复。",
        )}
        danger
