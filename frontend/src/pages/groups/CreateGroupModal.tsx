...
export default function CreateGroupModal({
  ...
  const toggle = (candidate: GroupMemberCandidate) => {
    setSelected((previous) =>
      previous.some((item) => item.participant_id === candidate.participant_id)
        ? previous.filter(
            (item) => item.participant_id !== candidate.participant_id,
          )
        : [...previous, candidate],
    );
  };
  ...
  const confirm = () => {
    ...
    onCreate(
      name.trim(),
      selected.map((candidate) => candidate.participant_id),
    );
  };
      ...
      <div className="group-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="group-candidate-list">
          ...
          {candidates.map((candidate) => {
            ...
            return (
              ...
              >
                <span
                  className={`group-avatar sm ${candidate.participant_type === "agent" ? "agent" : ""}`}
                >
                ...
                <div className="group-candidate-body">
                  <div className="group-candidate-name">
                    {candidate.display_name}
                  </div>
        ...
        <div className="group-create-footer">
          <span className="group-create-count">
            {t("groups.selectedCount", "已选 {{count}} 位", {
