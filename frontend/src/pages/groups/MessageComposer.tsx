...
export default function MessageComposer({
  ...
  const applyMention = (member: GroupMember) => {
    ...
    setMentionBindings((current) =>
      replaceMentionBinding(value, next, current, query.start, replacementEnd, {
  ...
  const submit = async () => {
    ...
    const mentionParticipantIds = liveMentionParticipantIds(
      value,
      mentionBindings,
    );
          ...
          {candidates.map((member, index) => (
            ...
            >
              ...
              {member.role_description && (
                <span className="group-mention-hint">
                  {member.role_description}
                </span>
              )}
            </button>
