...
const statusLabel = (s: string, t: any) => {
  switch (s) {
    ...
    case "creating":
      return t("dashboard.status.creating");
    case "disconnected":
...
function StatsBar({
  ...
  const totalTokensToday =
    tokenUsage?.today?.total_tokens ??
    agents.reduce((sum, a) => sum + (a.tokens_used_today || 0), 0);
  const cacheReadToday =
    tokenUsage?.today?.cache_read_tokens ??
    agents.reduce((sum, a) => sum + (a.cache_read_tokens_today || 0), 0);
  const cacheHitRate =
    totalTokensToday > 0
      ? Math.round((cacheReadToday / totalTokensToday) * 100)
      : 0;
    ...
    >
      ...
      <div>
        ...
        >
          ...
          {maxTokens > 0 && (
            <span style={{ opacity: 0.6 }}> / {formatTokens(maxTokens)}</span>
          )}
        </div>
        {!!agent.cache_read_tokens_today && (
          ...
          >
            Cache {formatTokens(agent.cache_read_tokens_today)} ·{" "}
            {usedTokens > 0
              ? Math.round((agent.cache_read_tokens_today / usedTokens) * 100)
              : 0}
            %
          </div>
        )}
...
export default function Dashboard() {
  ...
  const greeting =
    hour < 6
      ...
      : hour < 12
        ? "☀️ " + t("dashboard.greeting.morning")
        : hour < 18
          ? "🌤️ " + t("dashboard.greeting.afternoon")
          ...
          >
            ...
            <div style={{ maxHeight: "350px", overflowY: "auto" }}>
              {agents
                .sort((a, b) => {
                  const aActive =
                    a.status === "running" || a.status === "idle" ? 1 : 0;
                  const bActive =
                    b.status === "running" || b.status === "idle" ? 1 : 0;
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: "11px", color: "var(--text-tertiary)" }}>
                {t("dashboard.recentCount", { count: 20 })}
              </span>
            </div>
            <div
              style={{ padding: "4px", maxHeight: "320px", overflowY: "auto" }}
            >
